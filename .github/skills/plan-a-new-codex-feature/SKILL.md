...
- **A user works ENTIRELY in the `api/*`/`ports` abstraction and ATTACHES adapters only for concrete IO — any new helper that only touches core `api/*` types (codecs, handles, declared patterns, a core-layer interface) belongs in `api/*`, never in `adapters/*`, even when only one adapter exists today.** A recurring, confirmed mistake in this codebase: `ErrorPatternAs`/`HandleErrorPattern`/`Case` were originally placed per-adapter (`adapters/nethttp`/`adapters/mqtt5`/`adapters/zeromq`) though they touch only the core `ErrorPatternValuer` interface — for reqreply this meant 2 adapters carried byte-for-byte duplicated code. Fixed by moving both into `api/rest`/`api/reqreply`. Before adding ANY new exported function to a feature's adapter-side code, ask: "could this be written using only `errors.As`/a core-layer interface, with zero adapter-specific types?" If yes, it belongs in `api/*`. See `docs/concepts/ports-and-adapters.md`'s "Convenience helpers belong in `api/*`, not adapters" section and the `add-a-new-adapter` skill's Step 5d for the full rule.
