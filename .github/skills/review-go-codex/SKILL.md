...
| `api/reqreply/{middleware,middleware_declaration,property_param,transform}.go` | reqreply's codec-declared `Middleware[In,Out]` (D-0003 parity) + the "property" vocabulary axis: `Route.Use`/`HandleMW`/`ClientMW`, `PropertyParam`/`MergedPropertyParam[T]`/`NewPropertyParam`/`NewOptionalPropertyParam`, `WithRequestProperty`/`WithResponseProperty`, uniform conflict detection (`checkReqReplyParamConflicts`), `CheckCoverage` — see `docs/design/d-0003-codec-declared-middlewares.md`'s Addendum |
| `api/reqreply/observability.go` | `Observability[Req, Resp any](obs stats.Observer)` — the shipped, general-purpose (unpaired) `.HandleMW(nil, fn)`/`.ClientMW(nil, fn)` observer decorator, mirroring `adapters/nethttp.Observability`/`adapters/zeromq.Observability` but living in the CORE package (covers zeromq server+client and mqtt5 client with ONE implementation; mqtt5 server-side intentionally has none — see `docs/features/observer.md`'s reqreply section) |
...
14. Godoc & documentation-site reference integrity — **run this whenever the round follows an
    exported symbol removal/rename**; `go build`/`go vet`/`go test`/`staticcheck` catch none of it
...
- **adapters/mcpgo** error behavior — **different from REST/events adapters**:
  ...
  - Handler errors matching a declared `mcp.ErrorPattern` → structured `mcp.NewToolResultStructured(...)` with `IsError: true` set manually; unmatched handler errors fall back to `mcp.NewToolResultError(err.Error())` unchanged.
  - Output encode failures → protocol-level Go error `(nil, err)` (server contract violation) — NEVER consults `ErrorPattern`.
...
- **Error-path ergonomics declarations** (`rest.ErrorPattern`, `events.ErrorChannel`,
  `reqreply.ErrorPattern`, `mcp.ErrorPattern`, `websocket.ErrorFrame`) are a DISTINCT layer from the
  typed sentinel errors above — they let USERS declare typed error RESPONSE payloads (codec-backed,
  matched via `errors.As` against application/domain error types), not go-codex's own internal error
  types. See checklist.md §13 for the full per-boundary matrix and rules. Do not conflate the two:
  a route/channel/tool with no `ErrorPattern`/`ErrorChannel` declared still returns the typed
  sentinel errors above unchanged — the declarative layer is purely additive.
...
- **A function that bundles multiple responsibilities (wiring + a validation/coverage check,
  say) can lose the non-obvious one silently when it's deleted and replaced.** If a round
  involves reviewing a recent old-API removal, write a reproduction test for each of the old
  function's LESSER-KNOWN side effects (not just its primary purpose) before trusting that the
  replacement preserves them — do not accept "the new entry point already does this" without
...
- **`ports` calling `Register` instead of `ClientHandle` is intentional, not a missed optimization.** `Register` is a strict superset of `ClientHandle` (adds duplicate-name checks for `reqreply`/`mcp`, unknown-param-name checks, path/topic codec validation, security scheme/global security population) — there is no case where `ClientHandle` would be preferable inside `ports`. Do not suggest reverting to `ClientHandle` for a "simpler" default path. **Both construction paths now apply declared `Formats`/`RequestFormats`/`SubscribeFormats`/`PublishFormats` identically** (`rest.Route`/`SSERoute`, `events.Channel`, `reqreply.Route`) — a previously-confirmed bug where `ClientHandle()` silently ignored these (always falling back to JSON) was fixed; do not reintroduce a `ClientHandle` construction path that skips the `rb.*Formats`/`cb.*Formats` type-assert-and-apply block `registerHandle`/`Register` already has.
...
- **`rest.DiagnosticObserver`/`Report*Errors` (7 functions) live in `api/rest` now, not per-adapter.** Confirmed byte-for-byte identical between `adapters/nethttp` and `adapters/chi`, moved into `api/rest/observability.go`; BOTH adapters' own copies were deleted entirely, both call the `rest.*` exports directly — zero adapter-owned wrapper remains (unlike `adapters/mqtt5.Observability`'s genuinely-adapter-specific remainder). Do not flag either adapter for "missing its own `diagnosticObserver`/`report*Errors`" — there is nothing adapter-specific left to keep. `ReportPathErrors` also unpacks `MissingPathVarError`/`InvalidPathParamError` (not just `PathParamError`) — do not flag the 3-type handling as redundant with `stats.ReportErrors`'s own generic walker; they serve different mechanisms (ctx-ferried `Diagnostic` vs. direct `RecordValidationError`), never both called for the same error at the same site.
- **`route.FirstSchemeName` lives in `route/security.go` now, not per-adapter.** Confirmed byte-for-byte identical `firstScheme`/`firstSchemeName` copies across `adapters/nethttp`, `adapters/chi`, `adapters/mqtt`, `adapters/mqtt5`, `adapters/zeromq` were deleted; all 5 now call `route.FirstSchemeName(reqs)` for `SecurityObserver.RecordSecurityRejection`'s scheme-name argument. Do not flag any adapter for lacking its own copy.
- **mqtt5/zeromq's `TopicMismatchError`/`InvalidTopicError` subscribe-side Observer coverage is now symmetric with mqtt v3's.** All 3 pub/sub adapters' subscribe dispatch call the SAME `reportInvalidTopicErrors`/`reportTopicMismatchErrors` reporter pair (mqtt v3 already had both; mqtt5/zeromq gained them) — both types have no `Unwrap()` reaching one of `stats.ReportErrors`'s handled cases, so a dedicated reporter is required alongside the generic call, on every adapter. zeromq's publish side calls only `reportInvalidTopicErrors` (its `BuildTopic` cannot produce a `TopicMismatchError`) — do not flag this as asymmetric, it is a structural impossibility, not a missed call site.
...
- **Error-path ergonomics (`ErrorPattern`/`ErrorChannel`/`ErrorFrame`/`ErrorAction`) design is intentional — do not flag (see checklist.md §13 for the full matrix):** `rest.ErrorPattern`/`events.ErrorChannel`/`reqreply.ErrorPattern`/`mcp.ErrorPattern` all match via `errors.As`, first-declared-rule-wins, with direct (assignable) and mapped (`mapFn`) modes; `rest.ErrorAction` is REST's OWN standalone type, NOT shared with `events.ErrorAction`/`websocket.ErrorFrame` (each API layer keeps parallel-but-independent vocabularies, same as `RouteMeta`/`ChannelMeta` — do not propose unifying them into one shared type); `rest.ErrorHandle`/`rest.ErrorLog` are BEHAVIORALLY IDENTICAL for REST (both fall through to `Options.ErrorHandler`, since REST has only one such hook, unlike adapters with a separate `OnError` callback) — kept as distinct values purely for cross-boundary vocabulary parity, not a missing distinction to implement; `reqreply.ErrorPattern` auto-generates the OLDER spec-only `ErrorReplyMeta`'s AsyncAPI entry in the SAME declaration — `ErrorReplyMeta` remains available unchanged for spec-only use, do not flag as dead code; adapter wiring ONLY touches handler/encode failure branches, NEVER decode failure (no business error exists yet at decode time); `mqtt.PublishAdapter`/`zeromq.PublishAdapter` mirror `mqtt5.PublishAdapter`'s `ErrorResponseFor`-consulting wiring exactly (all three shipped); SQL/Cache/File (`adapters/sql`/`adapters/redis`/`adapters/file`) have NO dedicated error-channel declaration type BY DESIGN — the existing `OnError func(error)` hook already covers "handle" generically, and composing it inline with `events.ErrorChannel.ErrorResponseFor` achieves the "respond"-equivalent with zero new API surface (do not propose `sql.ErrorChannel`/`redis.ErrorChannel`/`file.ErrorChannel`); `websocket.ErrorFrame` broadcasts to ALL connected sessions on match (no dedicated error-output topic exists on a socket — broadcast IS the notification path, not a design gap); `websocket.ErrorFrame[E,B]` declares its OWN codec-backed payload type B, independent of the socket's `Out` type (validated via `format.JSON(codec).Marshal` same as REST/events/reqreply/MCP — fixed after an initial version incorrectly reused `Out`'s codec); `ErrorFrameRule` is non-generic (payload pre-encoded at declaration time), so BOTH `DuplexSocketAdapterOptions.ErrorFrames` and `BroadcastSocketAdapterOptions.ErrorFrames` are plain `[]ErrorFrameRule` — no type erasure, no runtime type-assertion error type exists (do not look for an `ErrorFrameOptError`, it was removed as unreachable); `BroadcastSocketAdapter`'s `ErrorFrames` matches ONLY errors from the port's stream `Errors` channel — per-session write/encode failures stay `SocketError`-wrapped via `OnError` directly, never routed through `ErrorFrame` matching; `nethttp`/`chi` SSE helpers (`SSEFromStream`/`SSEFromHub`) correctly have NO declarative error-payload option — they take no bound route handle at their call site (bound later at `SSEHandler`), so there is nothing to consult `ErrorResponseFor` against; not a gap, an architectural difference from socket adapters which own a handle directly; `zeromq.ServeAdapter`/`mqtt5.ServeAdapter` (ToolPort) and `zeromq.ServeLatest`/`LatestAdapter` all delegate straight to the underlying `Serve` function (already `ErrorResponseFor`-aware), so a declared `reqreply.ErrorPattern` works for them with zero additional wiring — do not propose adding separate wiring; ports parity (`PluginRESTPattern`/`PluginEventPattern`) is proven via dedicated tests in `ports/port_test.go`, not just claimed.
- **Design guardrail: adapters implement wire protocols only — client-side/user-facing convenience helpers belong in `api/*`, never `adapters/*`, even when only one adapter exists today (checklist.md §13 has the full rule and per-boundary matrix — this is a pointer, not a restatement):** the concrete test is "could this be written using ONLY `errors.As`/a core-layer interface, with zero adapter-specific types?" — if yes, it's misplaced. `rest.ErrorPatternAs`/`HandleErrorPattern`/`Case` and `reqreply.ErrorPatternAs`/`HandleErrorPattern`/`Case` were moved OUT of `adapters/nethttp`/`adapters/mqtt5`/`adapters/zeromq` for exactly this reason (the reqreply pair was byte-for-byte duplicated across 2 adapters, confirming the misplacement). See `docs/concepts/ports-and-adapters.md`'s "Convenience helpers belong in `api/*`, not adapters" section and `docs/design/d-0005-error-handling.md`'s Topic 6 "Design guardrail" subsection.
