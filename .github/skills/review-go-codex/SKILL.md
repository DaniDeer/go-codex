...
- **`RESTPattern` is role-aware on single-codec ports — do not flag the shape divergence.** On `SourcePort[T]` it builds the HTTP-ingest shape `RouteHandle[T, struct{}]` (retrieve via `RESTHandle[T, struct{}]` — the type params express it, no dedicated accessor); on `SinkPort[T]` it builds the SSE shape `SSERouteHandle[struct{}, T]` (always GET — non-GET `Method` fails construction; retrieve via `SSEHandle[T]`, replay via `RegisterSSE`). The build fn takes an unexported `portRole` param. Only `nethttp.DrainCallAdapter` (outbound client, independent Resp codec) remains handle-first — by design, same asymmetry category as `ReadEachAdapter` enrichment.
