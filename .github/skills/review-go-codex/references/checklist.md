...
All must be `errors.As`-navigable and implement `slog.LogValuer` (none wrap an inner error, so none need `Unwrap`, except `MissingSocketError` which is terminal too). Bare `fmt.Errorf` in `api/reqreply/{builder,client,future}.go` or the two `reqreply_transport.go` adapter files without a typed wrapper is a finding.
...
| `api/reqreply` (req/reply) — escape hatch (`Serve`/`Call`/`CallHandle`) | `NewTopicParam[T]` (Req-side only) | `TopicParam` struct literals still work | `RouteHandle.DecodeMerged` (decode) + `MergeFields()` (encode) | server (`mqtt5.Serve` auto-merge) + client (`mqtt5.CallHandle`/`zeromq.CallHandle`) both covered | `mqtt5.CallHandle`/`zeromq.CallHandle` | ✅ nested-Req + Gob round trip test (`TestServeCallHandle_NestedReq_RoundTrip`) | ✅ SHIPPED, with a documented exception: `zeromq.CallHandle` is client-side ONLY — `zeromq.Serve` reads raw socket frames with no per-message topic string, architecturally cannot decode-merge server-side |
| `api/reqreply` (req/reply) — PREFERRED `Server`/`Client`+`Attach` (see `docs/design/d-0004-reqreply-workflow-simplification.md`) | n/a — reuses the SAME `NewTopicParam[T]` declarations as the escape hatch above (one route, either workflow) | `Serve`/`Call`/`CallHandle` above remain fully functional side-by-side | ⚠️ NOT SUPPORTED by the `mqtt5.AttachServer`/`AttachClient` and `zeromq.AttachServer`/`AttachClient`/`AttachRouterServer`/`AttachDealerClient` reflection shims — documented v1-scope limitation, confirmed in code: these shims always use plain JSON `handle.Decode`/`handle.Encode`, never `DecodeMerged`/`MergeFields` | server (`Server.Attach`+`Serve`) + client (`Client.Attach`+`Call`/`CallAsync`) both exist, but NEITHER side merges topic vars | none — a route needing merge-fields, per-call format overrides, or `ErrorPattern`-typed replies must use the escape hatch above instead | n/a (not supported) | ⚠️ KNOWN, DOCUMENTED GAP (not a bug) — do not flag a NEW finding for this specific gap unless a review round is ADDING new capability to the Attach shims without also closing it; DO flag if `examples/reqreply-api`'s demos ever claim merge-field support works through `Client.Attach` when it doesn't |
