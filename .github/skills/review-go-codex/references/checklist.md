...
| `adapters/websocket` | `websocket.ErrorFrame[E,B](codec, mapFn...) ErrorFrameRule` on `DuplexSocketAdapterOptions.ErrorFrames []ErrorFrameRule` (plain slice, non-generic rule type — declares its OWN codec, independent of the socket's `Out` type) | broadcast to all sessions (no dedicated topic — broadcast IS the notification path) | Full: shares `events.ErrorAction` (`.WithAction(events.ErrorHandle)` + `.WithHandle(func(error))`) |
