...
| `middleware`                   | Declarative, composable enrichment/enforcement vocabulary, replacing adapter-specific `Options.SecurityFunc`/`CallOptions.CredentialFunc` fields — Phase 1 REST-only so far (`api/rest` + `adapters/nethttp`/`chi`), see `docs/roadmap/declarative-middleware.md` for the full design and Phase 2 plan (events/reqreply/MCP/ports). TWO distinct types model server vs client roles: **`Middleware{Name, Fn any, Satisfies []string, Security *SecurityDeclaration, RequestParams/ResponseParams []any}`** — SERVER-side, the ONLY type that can contribute to a route's spec, attached via `rest.WithMiddleware`/`Route.Use`; **`ClientMiddleware{Name, Fn any}`** — CLIENT-side, structurally CANNOT contribute to the spec (no Security/RequestParams/ResponseParams fields exist on it at all), attached via `Route.UseClient`. Three-part vocabulary: `RequireScopes[Raw,Req](schemeName, scheme, scopes, codec, extract)` builds a `Middleware` with BOTH `Security` AND a verifying `Fn` (a route THIS codebase enforces); `DeclareSecurity(schemeName, scheme, scopes, codec)` builds a `Middleware` with `Security` ONLY, no `Fn`/empty `Satisfies` (a route documenting an EXTERNAL system's requirement this codebase never enforces — e.g. `examples/go-edge-models`'s Docker registry client; `Register()`-ing such a route still correctly fails with `MissingSecurityMiddlewareError`); a `ClientMiddleware` supplies the credential. `CheckScopes(reqs, granted)` performs the ONE centralized authorization check an adapter runs after merging every attached SERVER security `Fn`'s grants (never per-`Fn` — see the design doc's "L4" for why). `ContextField[V]`/`NewContextField`/`EnsureContextFields` — a codec-typed, ctx-carried value bus for cross-middleware data sharing. `MiddlewareShapeError`/`UnsatisfiedScopesError`/`ContextFieldNotPreparedError` — typed errors, shared by both server and client shape-validation. Two attachment points on `api/rest`: `rest.WithMiddleware(mws...) RouteOpt` (spec-relevant, at `NewRoute`/`.Register(builder)` time) and `nethttp.Register`/`Handler`/`SSEHandler`/`RegisterSSE`'s own variadic `mws` parameter (call-time-only, non-spec, e.g. `nethttp.ObservabilityMiddleware`). `rest.Route[Req,Resp].Use(mws ...middleware.Middleware) Route[Req,Resp]` / `rest.SSERoute[Req,Event].Use(...)` — chi/net-http-style **declaration-time sugar** for `WithMiddleware`, chainable and immutable (`.Use(mw1).Use(mw2)` ≡ `.Use(mw1, mw2)`); introduces NO new runtime mechanism. `rest.Route[Req,Resp].UseClient(mws ...middleware.ClientMiddleware) Route[Req,Resp]` — the CLIENT-side mirror; `RouteHandle.ClientMiddlewares` (populated by both `Register()` and `ClientHandle()`) is combined automatically by `nethttp.Call`/`CallHandle` with their own call-time variadic — mirrors `Register`/`Handler`'s `handle.Middlewares` + variadic combination exactly, on the client side. `Call`/`CallHandle`'s variadic parameter type is `...middleware.ClientMiddleware` (BREAKING from the original `...middleware.Middleware` Phase 1 shape — accepted, single internal consumer). | `codex`, `route`                                                           |
...
`nethttp.CallHandle`). `reqreply.NewTopicParam[T, V]` (Req-side only —
