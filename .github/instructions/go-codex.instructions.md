...
- `adapters/mqtt` wraps `ChannelHandle` for Paho MQTT. It imports `api/events`, `stats`, and `github.com/eclipse/paho.mqtt.golang`.
  ...
  - `SubscribeOptions{TopicFilter string, QoS byte, OnError func(SubscribeError), Observer stats.Observer, SecurityFunc func(ctx, pahomqtt.Message, []route.SecurityRequirement) error}` — zero value is safe (nil `OnError` discards errors, nil `Observer` defaults to `NoopObserver`).
  ...
  - `PublishOptions[T]{Observer stats.Observer, CredentialFunc func(ctx, msg *T, reqs []route.SecurityRequirement) error}` — zero value is safe (nil `Observer` defaults to `NoopObserver`). Observer also type-asserts to `stats.TraceObserver` for distributed tracing spans. `CredentialFunc` mutates `msg` directly to embed a credential as an ordinary payload field (MQTT 3.1.1 has no User Properties, unlike `mqtt5.PublishOptions.CredentialFunc`'s protocol-native return).
