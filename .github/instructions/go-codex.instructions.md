...
| `middleware`                   | Declarative, composable enrichment/enforcement unit (`Middleware{Name, Fn any, Satisfies []string, Security *SecurityDeclaration, RequestParams/ResponseParams []any}`), replacing adapter-specific `Options.SecurityFunc`/`CallOptions.CredentialFunc` fields — Phase 1 REST-only so far (`api/rest` + `adapters/nethttp`/`chi`), see `docs/roadmap/declarative-middleware.md` for the full design and Phase 2 plan (events/reqreply/MCP/ports). `RequireScopes[Raw,Req](schemeName, scheme, scopes, codec, extract)` builds a security `Middleware` that is BOTH the spec declaration AND the runtime authentication step; `CheckScopes(reqs, granted)` performs the ONE centralized authorization check an adapter runs after merging every attached security `Fn`'s grants (never per-`Fn` — see the design doc's "L4" for why). `ContextField[V]`/`NewContextField`/`EnsureContextFields` — a codec-typed, ctx-carried value bus for cross-middleware data sharing. `MiddlewareShapeError`/`UnsatisfiedScopesError`/`ContextFieldNotPreparedError` — typed errors. Two attachment points on `api/rest`: `rest.WithMiddleware(mws...) RouteOpt` (spec-relevant, at `NewRoute`/`.Register(builder)` time) and `nethttp.Register`/`Handler`/`SSEHandler`/`RegisterSSE`'s own variadic `mws` parameter (call-time-only, non-spec, e.g. `nethttp.ObservabilityMiddleware`). `rest.Route[Req,Resp].Use(mws...) Route[Req,Resp]` / `rest.SSERoute[Req,Event].Use(...)` — chi/net-http-style **declaration-time sugar** for `WithMiddleware`, chainable and immutable (`.Use(mw1).Use(mw2)` ≡ `.Use(mw1, mw2)`); introduces NO new runtime mechanism — `Register`/`ClientHandle` still perform the SAME single application pass regardless of whether middleware arrived via `Use` or the `WithMiddleware` opt directly. | `codex`, `route`                                                           |
