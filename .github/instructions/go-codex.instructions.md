...
| `adapters/file`                | Stdlib-only file bridge — no external dependencies; **sources**: `ScanStream[T](ctx, path, fmt, srcOpts) (Stream[T], error)` (reads file line by line; decode errors → `StreamDecodeError`; open/read errors → `ScanError{Path,Err}`); `WatchStream(ctx, dir, interval, srcOpts) Stream[string]` (polls dir with `os.ReadDir`; emits absolute path of each new file; `WatchError{Dir,Err}` on ReadDir failure; no duplicate emission); **intermediate (enrichment)**: `ReadEachStream[In,T,Out](ctx, format.File[T], src Stream[In], varsFor func(In)map[string]string, combine func(In,T)Out, ReadEachStreamOptions) Stream[Out]` — reads a complete typed file for each upstream item and combines result; `varsFor` maps each item to path template vars; `combine` pairs original item with file content; read errors → `ReadError{Err}` in `Stream.Errors` + `OnError`; upstream errors forwarded; `ReadEachStreamOptions{OnError, Observer, FileOptions, Buffer}`; **sinks**: `DrainWrite[T](ctx, w io.Writer, src, fmt, DrainWriteOptions)` (encodes each item + separator; encode/write errors → `WriteError{Path,Err}` via OnError; `DrainWriteOptions{OnError, Path, Separator, Observer stats.Observer}` — Observer receives `RecordValidationError` for encode failures via `stats.ReportErrors`); default separator `"\n"` (NDJSON); `TapWriteFile[T](ctx, format.File[T], src, vars, TapWriteFileOptions) Stream[T]` — writes each item as a complete typed file (overwrite), stream continues flowing; use when file write is one of multiple side-effects; `DrainWriteFile[T](ctx, format.File[T], src, vars, DrainWriteFileOptions)` — writes each item as a complete typed file (terminal sink); both options structs carry `OnError func(error)`, `Observer stats.Observer`, `FileOptions format.FileOptions`; **errors**: `ScanError{Path,Err}`, `WatchError{Dir,Err}`, `WriteError{Path,Err}`, `ReadError{Err}` — all `slog.LogValuer` + `Unwrap()` | `format`, `stream`, stdlib only                                             |
| `stream`                       | Reactive stream pipeline over typed Go channels — bridges MQTT/ZeroMQ sources with `forge.Function[In,Out]`; **`Stream[T]{Values <-chan T, Errors <-chan error}`** — explicit error channels (idiomatic Go); **sources**: `From[T](ctx, <-chan T) Stream[T]`, `FromCodec[T](ctx, <-chan []byte, format.Format[T], SourceOptions) Stream[T]` (accepts any format — JSON, YAML, TOML, custom), `Single[T](ctx, v T) Stream[T]` (one-shot stream; emits v once, then closes; errors channel never written; used as per-request pipeline entry point in `PipelineHandlerFunc` and `AsPipelineFunc`); **`BroadcastHub[T]`** — N-subscriber fan-out; `NewBroadcastHub(ctx, src Stream[T], bufPerSubscriber int) *BroadcastHub[T]`; `Subscribe() Stream[T]` (returns buffered subscriber stream), `Unsubscribe(Stream[T])` (removes subscriber; channel closed when hub exits); hub goroutine fans out both values AND errors to all subscribers; non-blocking fan-out (slow subscriber drops items silently); used by `SSEFromHub` in nethttp/chi; **transforms**: `Apply[In,Out](ctx, Stream[In], *forge.Function[In,Out], ApplyOptions) Stream[Out]` (calls `forge.Function.ApplyContext` per item; fires `stats.StreamObserver.RecordStreamItem` if observer implements it; errors wrapped in `StreamApplyError`), `Filter[T]`, `Tap[T]` (domain event observation — calls `onValue(T)` without transforming; distinct from infrastructure metrics), `MapErr[T]` (recover/reclassify/silence errors), `Retry[T]` (alias for MapErr with descriptive name for retry pattern); **fan-in/out**: `Merge[T]`, `Tee[T]`; **time**: `Buffer[T]`, `Debounce[T]`, `Throttle[T]`; **sinks**: `Drain[T]` (drains both channels safely in one select loop), `Collect[T]`; **combine**: `CombineLatest2[A,B,Out]`; **errors**: `StreamDecodeError{Source,Err}` + `StreamApplyError{Function,Err}` — both `Error()`, `Unwrap()`, `slog.LogValuer`; **observer**: `stats.StreamObserver.RecordStreamItem` type-asserted from `ApplyOptions.Observer`; `nil` Observer → `stats.NoopObserver`; **topology**: `Topology` + `WithApply[In,Out](topo, fn)` free function (captures forge.FunctionSpec.Hash) + `TopologySpec`; `render/stream.Render(TopologySpec)([]byte,error)` — stream topology YAML (analogous to render/pipeline); **TraceObserver**: `Apply` wraps each item in a child span when `opts.Observer` implements `stats.TraceObserver`; no external dependencies | `codex`, `format`, `forge`, `stats` |
| `ports`                        | **Protocol-agnostic IO enforcement points** for inside-out pipeline wiring. Declare in domain/pipeline code (zero adapter imports); bind concrete transports in main.go. **Three port types**: `SourcePort[T]` (external → pipeline, fan-in: multiple adapters merged), `SinkPort[T]` (pipeline → external, fan-out: broadcast to all adapters), `IOPort[Req,Resp]` (intermediate IO transform, exactly one adapter). **`IOParam`** — protocol-agnostic parameter declaration with codec (maps to `PathParam`/`TopicParam`/`FilePathParam`/`UserPropertyParam` at adapter binding time): `IOParam{Name, Description, Codec *codex.Codec[string], Required bool}.WithCodec(c)` mirrors `rest.PathParam.WithCodec`. **`PortOptions{Params []IOParam, Buffer int, Observer stats.Observer}`** — configures all port constructors. **Constructors**: `NewSourcePort[T](name, codec, PortOptions) *SourcePort[T]`, `NewSinkPort[T](name, codec, PortOptions) *SinkPort[T]`, `NewIOPort[Req,Resp](name, reqCodec, respCodec, PortOptions) *IOPort[Req,Resp]`. **Adapter interfaces** (implemented by binding constructors in each adapter package): `SourceAdapter[T]` (`Activate(ctx, dst chan<-T, errs chan<-error)` + `AdapterName() string`), `SinkAdapter[T]` (`Activate(ctx, src Stream[T])` + `AdapterName()`), `IOAdapter[Req,Resp]` (`Transform(ctx, src Stream[Req]) Stream[Resp]` + `AdapterName()`). **Test helpers**: `ChanSourceAdapter[T](<-chan T)`, `ChanSinkAdapter[T](chan<-T)`, `FuncIOAdapter[Req,Resp](func(ctx,Req)(Resp,error))`. **Errors**: `PortBindError{Port,Adapter,Err}` (`Unwrap()` + `slog.LogValuer`), `PortNoAdapterError{Port}` (`slog.LogValuer`). **Each adapter package has a binding.go**: `mqtt5.SubscribeAdapter/PublishAdapter/CallAdapter`, `mqtt.SubscribeAdapter/PublishAdapter`, `nethttp.IngestAdapter/SSEAdapter/CallAdapter/PollAdapter/DrainCallAdapter`, `zeromq.SubscribeAdapter/PublishAdapter/CallAdapter`, `file.ScanAdapter/WatchAdapter/ReadEachAdapter/DrainWriteAdapter/DrainWriteFileAdapter`, `sql.QueryAdapter/QueryEachAdapter/DrainInsertAdapter`. Stream bridge functions (`SubscribeStream`, `DrainPublish`, `CallStream`, etc.) are deprecated — use port binding constructors instead. Non-stream adapter functions (`Subscribe`, `Publish`, `Call`, `Serve`, `Handler`) remain fully supported for standalone (non-pipeline) use. | `codex`, `stats`, `stream`                                                 |
