...
- `adapters/nethttp` wraps `RouteHandle` for `net/http`. It imports `api/rest`, `net/http`, `format`, and `stats`.
  ...
  - **Pipeline error-channel status mapping**: `PipelineHandler`/`RegisterPipeline` honor route-declared `rest.PipelineErrorStatus[E](status)` mappings (first match wins via `errors.As`). Unmatched pipeline errors remain 500. `PipelineNoResponseError` defaults to 503 unless explicitly remapped by a route rule.
