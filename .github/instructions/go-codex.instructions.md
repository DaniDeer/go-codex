...
- Prefer defining `Codec()` as a package-level function `func Codec() codex.Codec[MyType]` (returned via a value receiver method, e.g. `func (MyType) Codec() codex.Codec[MyType] { return myTypeCodec }`) when the type is a value type with no per-instance state — the common case.
