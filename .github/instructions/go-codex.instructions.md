...
| `validate`        | Reusable `Constraint` functions: numbers, strings, format, bytes (`MaxBytes`, `MinBytes`, `HasPrefix(prefix []byte)` — magic-byte check, produces `ConstraintError`) | `codex`, `schema`                |
