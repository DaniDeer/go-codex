...
| `format`          | Bridges `Codec[T]` to wire formats: JSON, YAML, TOML, Gob (binary), streaming; `FromEnv[T]` for schema-driven env var loading; `NewStreamed` for chunked/SSE writes; `Gob[T](codec)` uses `marshalTyped`/`unmarshalTyped` path (bypasses `map[string]any`), ContentType `"application/gob"`, constraints enforced on both marshal and unmarshal — suitable for Go-to-Go binary communication, not REST content negotiation | `codex`, `schema`, external libs |
...
| `api/mcp`         | Transport-agnostic MCP server builder; Tools, Resources, and Prompts follow the same declare → register → handle pattern as `api/rest` and `api/events`; `NewTool[In,Out](name, inputCodec, outputCodec, opts...)`, `NewResource[T](uriTemplate, codec, opts...)`, `NewPrompt(name, opts...)` — all return declarative value types; `Tool.Register(b)` → `ToolHandle[In,Out]` with `Decode(any)(In,error)` + `Encode(Out)([]byte,error)` + `InputSchema`/`OutputSchema json.RawMessage`; `ResourceHandle[T]` has `Encode`, `BuildURI`, `ValidateURIVars` (uses `api/internal.BuildFromTemplate`); `PromptHandle` has `ValidateArgs(map[string]string)error` + `Args []PromptArg`; `Builder.MCPSpec()` returns `*MCPSpec{Name,Version,Tools,Resources,Prompts}` (static JSON doc analogous to OpenAPI/AsyncAPI); `ToolMeta{Description,Tags}`, `ResourceMeta{Name,Description,MimeType}`, `PromptMeta{Description}` implement opt interfaces (mirrors `RouteMeta`/`ChannelMeta`/`FunctionMeta`); `ResourceParam{Name,Description,Codec}.WithCodec(c)` and `PromptArg{Name,Description,Required,Codec}.WithCodec(c)` mirror `TopicParam.WithCodec`; typed errors: `ToolInputError{Name,Err}`, `ToolOutputError{Name,Err}`, `ResourceEncodeError{URI,Err}`, `ResourceURIVarError{Name,Value,Err}`, `MissingResourceURIVarError{Name}`, `PromptArgError{Name,Err}`, `MissingPromptArgError{Name}` — all `errors.As`-navigable | `codex`, `schema`, `render/jsonschema`, `api/internal` |
