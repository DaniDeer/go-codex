...
- `MaybeField` is built DIRECTLY on `omitempty.go`'s existing unexported `sparseField[T,F]`/`sparseFieldCodec[T]` machinery (`Maybe[V].IsSet()` negated is a perfect `isEmpty` predicate) — **zero changes to `object.go`/`Struct` were needed**, confirming that mechanism was already general enough. An unexported `maybeFieldCodec[V](inner Codec[V]) Codec[Maybe[V]]` wraps the field's codec so Decode produces `Just(value)` and Encode unwraps via `.Get()` (only ever reached for a `Just`, since `sparseField.encodeSparse` already skips `codec.Encode` entirely for `Nothing`).
- `MaybeMap` (not `Map`) — named to avoid colliding with the EXISTING `codex.Map[K,V](keyCodec, valueCodec) Codec[map[K]V]` map-codec constructor in `stringmap.go`. Like `forge.NewFunction[In, Out]`, Go generic methods cannot introduce a new type parameter (`R`), so `MaybeMap` is a free function, not a method.
- `Maybe[T]` stays codec-agnostic (no `Codec[T]` of its own, no mutex) — validation is owned entirely by the enclosing field's `Codec[V]`, exactly like every other `Struct` field constructor; a zero-initialized `Maybe[V]{}` (`Nothing`) needs no constructor call at all, unlike `Immutable`/`Mutable` which both REQUIRE a codec at construction.
...
- Deferred, NOT designed here (see `docs/roadmap/maybe-nullable-and-codec.md`, kept as an open "idea only" roadmap entry): a three-state `Maybe[Nullable[T]]` composition (absent/present-null/present-value — the RFC 7396 JSON Merge Patch shape) and a general-purpose PUBLIC `codex.Codec[Maybe[T]]` (the unexported `maybeFieldCodec` helper is 90% of this already). Also see `docs/roadmap/optional-mutable.md` for `OptionalMutable[T]` — a DIFFERENT, not-yet-built idea (`Mutable[T]` that may start unset) that sounds similar but solves an opposite problem; see that doc's own comparison table.
