...
Remaining low-priority items (SSE merge support, `adapters/websocket`'s
per-connection path vars, a shared non-wildcard topic-template core across
mqtt/mqtt5/zeromq/ports.File) are tracked in
`docs/roadmap/merge-field-remaining-gaps.md`/
`docs/roadmap/file-cache-merge-field-gaps.md` — forward-looking backlog,
not blockers.
