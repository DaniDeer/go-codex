...
| `adapters/websocket`           | Server-side WebSocket adapter — typed, codec-validated frame streams over persistent bidirectional connections (completes the HTTP story: IngestAdapter=in, SSEAdapter=out, WS=both). Wraps `github.com/gorilla/websocket` behind **narrow interfaces**: `Socket{ReadMessage()([]byte,error), WriteMessage([]byte) error, Close() error}` + `Upgrader{Upgrade(w,r)(Socket,error)}`; `NewUpgrader(UpgraderOptions{Subprotocols, PingInterval /*30s*/, ReadLimit /*1MiB*/, CheckOrigin}) Upgrader` is the ONLY gorilla touchpoint — keepalive (ping/pong deadlines) is shim-owned; tests use hand-written fakes + one gorilla loopback test; **`Hub`** — session registry constructed in main (`NewHub(buffer /*<=0 → 16*/)`), passed to adapters: `SessionInfo(ports.Session) (map[string]string, bool)` (upgrade-time path vars, e.g. which {room}), `Sessions() []ports.Session`; per-session buffered outbound queue — full queue DROPS the frame for that session only (`SocketError` wrapping `ErrFrameDropped` sentinel; slow client never blocks pipeline/other sessions); **port adapters** (binding.go): `IngestSocketAdapter[T](mux Mux, hub, upgrader, handle ports.Socket[T,struct{}], IngestSocketAdapterOptions{Observer}) ports.SourceAdapter[T]` (inbound-only: frames from ALL clients feed the port), `BroadcastSocketAdapter[T](mux, hub, upgrader, handle ports.Socket[struct{},T], BroadcastSocketAdapterOptions{OnError,Observer}) ports.SinkAdapter[T]` (WS sibling of SSE; inbound discarded), `DuplexSocketAdapter[In,Out](mux, hub, upgrader, handle ports.Socket[In,Out], DuplexSocketAdapterOptions{Observer}) ports.DuplexAdapter[In,Out]` (session-tagged inbound `Framed[In]` + targeted/broadcast outbound; unknown target session / dropped frame → `SocketError` on port Errors); `Mux` iface = `Handle(pattern, http.Handler)` subset of http.ServeMux (Go 1.22 patterns, "GET /live/{room}"); handlers registered in Activate; upgrade validated once per connection (ALL template vars extracted for SessionInfo, declared PathParam codecs validated via handle.Route → 422 + RecordValidationError("path"); RecordRequest GET/101/4xx per upgrade attempt); frame decode failure → port Errors + "payload" reports, CONNECTION STAYS OPEN; ctx cancel → closeAll + wg.Wait (no sends after Activate returns); **errors**: `SocketError{Path, Session, Op /*upgrade/read/write/close*/, Err}` (`Error()`, `Unwrap()`, `slog.LogValuer`) + `ErrFrameDropped` sentinel; **observer**: NO new extension — `RecordRequest` per upgrade, `RecordSubscribe` per inbound frame, `RecordPublish` per outbound frame; nil → `stats.ObserverFromContext(ctx)`; **semantics**: at-most-once, no retained value — offline/reconnecting clients LOSE frames (pair with LatestPort for state-on-connect); NOT an MQTT broker — MQTT-over-WS is the MQTT client's transport option (ws:// broker URL to paho); **client-side dial adapters** (client.go): `Dialer{Dial(ctx, url) (Socket, error)}` + `NewDialer(DialerOptions{Subprotocols, RequestHeader, PingInterval, ReadLimit})` gorilla dial shim; `DialSourceAdapter[T](dialer, baseURL, vars, handle ports.Socket[T,struct{}], DialAdapterOptions{MaxBackoff /*30s cap*/, Observer}) ports.SourceAdapter[T]` (consume external feed), `DialSinkAdapter[T](… ports.Socket[struct{},T] …) ports.SinkAdapter[T]`, `DialDuplexAdapter[In,Out](… ports.Socket[In,Out] …) ports.DuplexAdapter[In,Out]` (ONE dialed connection; inbound tagged with session GENERATION "c1","c2",… — a generation change IS the reconnect marker; outbound Session ignored, one peer); URL = baseURL + handle.Route.BuildPath(vars) (PathParam codecs validate); **reconnect BY DESIGN — no silent loss**: auto-reconnect exp backoff 250ms→MaxBackoff (reset after a connection with traffic), EVERY failed dial (`Op:"dial"`) and EVERY drop (`Op:"read"`) → SocketError on port Errors; outbound frames while down (INCLUDING initial connection establishment) DROPPED with ErrFrameDropped — pump/buffer upstream if first frames matter; RecordRequest per dial attempt (101/503) | `api/rest`, `ports`, `stats`, `stream`, `github.com/gorilla/websocket` |
| `ports`                        | **Protocol-agnostic IO enforcement points** for inside-out pipeline wiring. Declare in domain/pipeline code (zero adapter imports); bind concrete transports in main.go. **Seven port types**: `SourcePort[T]` (external → pipeline, fan-in: multiple adapters merged), `SinkPort[T]` (pipeline → external, fan-out: broadcast to all adapters; ALSO supports the request-fed lifecycle `Start(ctx)`/`Push(ctx, v) error`/`Close() error` — port-owned channel + drain goroutine through the same broadcast path as `Feed`; `Push` blocks with backpressure, returns `PortNotStartedError{Port,Op}` before Start/after Close/on a Feed-driven port (the two lifecycles are mutually exclusive), `ctx.Err()` when cancelled while blocked; `Start` is a no-op if already owned, `Close` idempotent and waits for in-flight Push + adapter drain), `IOPort[Req,Resp]` (intermediate IO transform, exactly one adapter), `ToolPort[In,Out]` (server-side request/response — the complement of `IOPort`; `SetPipeline(fn)` once, `Bind` to multiple transports concurrently; `PortNoPipelineError{Port}` if `Bind` called before `SetPipeline`), `LatestPort[T]` (reactive cache: `Feed(ctx, src)` drains a stream into an atomic cell — src errors dropped, cache OUTLIVES the stream; `Bind(ctx, LatestAdapter[T]) error` fan-out serving — adapter contract `Serve(ctx, latest func() (T, bool)) error` MAY return after registration or block until ctx done, run in a supervised goroutine; `Latest() (T, bool)` programmatic read; Patterns build with request codec `codex.Struct[struct{}]()` — `RESTPattern`/`ReqReplyPattern`/`MCPPattern`; empty-cache behavior is per-transport: HTTP 503+`NoLatestValueError`, zeromq error reply, MCP error result). `DuplexPort[In,Out]` (bidirectional session boundary — external peers send In frames, receive Out frames over persistent identified sessions; `Framed[T]{Session ports.Session, Payload T}` tags every frame — inbound carries the sender, outbound targets one session or broadcasts when Session is zero (""); `Bind(ctx, DuplexAdapter[In,Out]) error` EXACTLY ONE adapter (second → `PortBindError`, IOPort precedent — session identity across transports unresolved); `Inbound(ctx) Stream[Framed[In]]` (port-owned channels, call once, after Bind); `Feed(ctx, src Stream[Framed[Out]])` blocks until src ends, then closes outbound channels signalling the adapter; adapter contract `Activate(ctx, dst chan<- Framed[In], errs chan<- error, src Stream[Framed[Out]]) error` — must not close dst/errs, must drain src; session routing composes with stream.GroupBy by Framed.Session; only `SocketPattern` accepted — ANY other pattern kind fails construction). **`PipePort[T]`** (pipeline stage segmentation, declared flexibly then never mutated at runtime — a thin wrapper over `SourcePort`/`SinkPort`/`gstream`, not a new adapter model: `ChainStream[In,Out](ctx, from *PipePort[In], transform func(gstream.Stream[In]) gstream.Stream[Out], to *PipePort[Out])` free function is the GENERAL stage connector — wraps `from.Stream(ctx)` + arbitrary transform + `gstream.Drain` + `to.Push`, accepting any number of chained `Map`/`Filter`/etc. steps; `Chain[In,Out](ctx, from, fn func(In)(Out,error), to)` is `ChainStream`'s single-Map SPECIAL CASE — both call a shared internal `chainWire` engine, each first recording its own `ChainEdge` (see below) so spec generation stays accurate per entry point; a multi-step sub-pipeline needs no hand-written wrapper function, just a direct `ChainStream` call with the SAME `(ctx, from, to)` shape; `Stream(ctx) gstream.Stream[T]` available directly for cases neither covers (e.g. fanning one stream into several independently-wired downstream pipes); `Push(ctx, v) error` feeds items into the pipe at ANY time — before or after `Connect` (pushCh allocated eagerly in `NewPipePort`, items buffer until Connect's consumer drains them; returns `ctx.Err()` only on cancellation, no "not connected" error state); secondary IO use: `InputPort(name) *SourcePort[T]` for adapter fan-in, `OutputPort(name) *SinkPort[T]` for adapter fan-out (same name → same instance, `pipe/in/name` / `pipe/out/name` qualified, built with `PortOptions{Buffer,Params,Observer}` only — no `Patterns` forwarding); `InputPortWithPatterns(name string, patterns []Pattern) (*SourcePort[T], error)`/`OutputPortWithPatterns(name string, patterns []Pattern) (*SinkPort[T], error)` — additive overloads that DO forward `Patterns` to the inner port (needed for real protocol adapters — mqtt5, nethttp SSE, etc. — bound through a PipePort's IO-bridging use case), AND forward `NewPipePort`'s `PortOptions.{REST,Event,ReqReply,MCP}Builder` (stored on the pipe at construction, unused otherwise) so a Pattern needing a shared, application-wide builder registers against THAT builder — not a private single-use one it would otherwise fall back to; fallible (`PatternRegisterError` on an invalid `Pattern`), same name→same instance rule; `Connect(ctx)` starts the hub goroutine, idempotent-safe (second call is a no-op reported via observer, not a duplicate-read bug), fully instrumented — `RecordSubscribe` on the Push-consumer path, `RecordPublish` per fan-out destination (success and failure); `Done() <-chan struct{}` closes only after Connect's internal goroutines fully exit (teardown-complete signal, NOT closed if Connect was never called) — pairs with `app.App.Supervise(name, start func(ctx) <-chan struct{})` for lifecycle supervision of a fire-and-forget `PipePort` without racing `ctx.Done()` against real completion; ONE ordering rule: register InputPort/OutputPort/Stream/Chain/ChainStream before that pipe's Connect — Chain/ChainStream only need to precede the UPSTREAM pipe's Connect, downstream Connect may happen before or after; `Chain`/`ChainStream` wrap their edge-setup (not per-item) in a `"pipe.chain"` `TraceObserver` span; no Patterns on the pipe itself — schema comes from individual SourcePort/SinkPort ports, or from `InputPortWithPatterns`/`OutputPortWithPatterns` when IO-bridging). **Pipeline spec generation — derived, not hand-typed** (`ports/pipeline_spec.go`): `SourcePort.BoundAdapters()`/`SinkPort.BoundAdapters() []string` track every bound adapter's real `AdapterName()` at `Bind` time (previously discarded after transient use); `ChainEdge{Kind, To, Func}` recorded by `Chain`/`ChainStream` on their `from` pipe, `Func` derived via `reflect.ValueOf(fn).Pointer()` + `runtime.FuncForPC` — real Go function identity (e.g. `"main.validateReading"`), honestly closure-opaque for inline `ChainStream` transforms, never fabricated; `PipePort.Buffer()`/`InputAdapters() map[string][]string`/`OutputAdapters() map[string][]string`/`OutEdges() []ChainEdge` expose this as accessors; `PipeSpecSource` interface (`Name`, `Buffer`, `InputAdapters`, `OutputAdapters`, `OutEdges` — none reference `T`) is satisfied by `*PipePort[T]` for ANY `T`, so `PipelineSpec(title, version string, pipes ...PipeSpecSource) gstream.TopologySpec` accepts a heterogeneous pipeline (different payload types per stage) in one call — only `title`/`version`/pipe ordering are caller-supplied, everything else is read from the pipes themselves. **Constructors — all return `(*Port, error)`**: `NewSourcePort[T](name, codec, PortOptions)`, `NewSinkPort[T](name, codec, PortOptions)`, `NewIOPort[Req,Resp](name, reqCodec, respCodec, PortOptions)`, `NewToolPort[In,Out](name, inCodec, outCodec, PortOptions)`, `NewLatestPort[T](name, codec, PortOptions)`, `NewDuplexPort[In,Out](name, inCodec, outCodec, PortOptions)`, `NewPipePort[T](name, codec, PortOptions)` — wrap package-level declarations with `codex.Must(...)`. **`Pattern` — the primary declaration surface** for handle-backed adapters: `RESTPattern{Method, Path, Opts []rest.RouteOpt}` (HTTP — ROLE-AWARE: `IOPort`/`ToolPort` build `RouteHandle[Req,Resp]` from the codec pair; `LatestPort` builds `RouteHandle[struct{},T]`; `SourcePort[T]` builds the HTTP-ingest shape `RouteHandle[T,struct{}]` (request body = payload, empty response — pairs with nethttp/chi.IngestAdapter; retrieve via `RESTHandle[T,struct{}]`); `SinkPort[T]` builds the SSE shape `SSERouteHandle[struct{},T]` (always GET — a non-GET Method fails construction with `PatternRegisterError`; pairs with nethttp/chi.SSEAdapter; retrieve via `SSEHandle[T]`); the outbound-client sink `DrainCallAdapter` needs an independent Resp codec and stays handle-first), `EventPattern{Topic, Opts []events.ChannelOpt}` (pub/sub — `SourcePort`/`SinkPort` only), `ReqReplyPattern{Topic, Opts []reqreply.RouteOpt}` (request/reply — `IOPort`/`ToolPort` only), `MCPPattern{Name, Opts []apimcp.ToolOpt}` (MCP tool — `ToolPort` only), `FilePattern{Path, Format FileFormatKind, CustomFormat any, Opts []ports.FileOpt}` (typed file — `SinkPort` builds `ports.File[T]` from the payload codec, `IOPort` builds `ports.File[Resp]` from the RESPONSE codec; `FileFormatKind`: `FileFormatJSON` default/`FileFormatYAML`/`FileFormatTOML` applied to the port's codec — a generic `format.Format[T]` can't sit in the non-generic struct; **`CustomFormat`** — general escape hatch: a pre-built `format.Format[T]` value (type-erased `any`), overrides `Format` when non-nil, type-asserted against T at build time — mismatch returns `PatternRegisterError` (construction is infallible ONLY on the enum-only path; `CustomFormat` can fail); use for Gob/binary/PNG/PDF/protobuf/any format `format.New`-family constructors produce — no dedicated `FileFormatGob` enum value BY DESIGN: J/Y/T share one construction shape (map[string]any intermediate via codec Encode/Decode), Gob is `NewTyped`-style (direct typed value, no intermediate) and belongs in the general escape hatch, not the closed enum), `SQLPattern{Table, Op string}` (SQL — **metadata-only by design**: query text/placeholders are driver-specific typed closures, no handle, no spec; propagated to adapters via ctx — `WithSQLMeta(ctx, m)`/`SQLMetaFromContext(ctx) (SQLPattern, bool)` mirror `WithParams`, wrapped automatically by every port `Bind` and `IOPort.Connect`), `CachePattern{Key string /*template "user:{id}"*/, TTL time.Duration /*0 = no expiry*/, Format FileFormatKind, CustomFormat any, Opts []CacheOpt}` (key/value cache — accepted on `IOPort` (cached value = RESPONSE codec, mirrors FilePattern), `SinkPort` (payload codec), `LatestPort` (value codec, var-free key); REJECTED on `SourcePort`/`ToolPort` at construction with `PatternRegisterError{Kind:"cache"}` — a cache neither produces a stream nor is a tool surface; builds `Cache[T]{Key, TTL, Format format.Format[T]}` with `BuildKey(vars map[string]string) (string, error)` template expansion — missing var → `CacheKeyError{Key, Var}` (`slog.LogValuer`, no Unwrap); retrieve with `CacheHandle[T]`; `CustomFormat` same escape-hatch contract as `FilePattern.CustomFormat`, overrides `Format`; **`Opts []CacheOpt`** — sealed interface, currently only `CacheKeyParam{Name, Description, Codec *codex.Codec[string]}.WithCodec(c)` implements it, mirrors `ports.FilePathParam`/`ports.NewFile` exactly (`ports` is to `Cache[T]` what `format` is to `File[T]`); a declared codec validates the key var's VALUE at `BuildKey`/`ValidateKeyVars` time — codec rejection returns `CacheKeyParamError{Key, Var, Value, Err}` (new, `slog.LogValuer` + `Unwrap`; distinct from `CacheKeyError`'s missing-var case, which is unchanged); `KeySchemas() map[string]schema.Schema` mirrors `File.PathParamSchemas` for future spec tooling (no renderer consumes it yet); a var with no matching `CacheKeyParam` (or a nil `Codec`) substitutes unvalidated, byte-identical to pre-feature behavior; **`NewCache[T](key, format.Format[T], opts ...CacheOpt) Cache[T]`** is the standalone constructor mirroring `ports.NewFile` — build a `Cache[T]` directly with no port/pipeline involved and pass it straight to `redis.GetAdapter`/`SetAdapter`/`DrainSetAdapter`/`Seed`, same codec validation (value AND key vars) either way; **`redis.GetMerged`/`redis.SetHandle`** are the single-call convenience mirroring `File.ReadMerged`/`ports.WriteHandle` for the cache boundary — `GetAdapter` calls `GetMerged` internally, `SetAdapter`/`DrainSetAdapter`'s `keyFn` may be `nil` to derive key vars from the item's own merge fields automatically), `SocketPattern{Path /*upgrade path template "/live/{room}"*/, Subprotocols []string, Format FileFormatKind, CustomFormat any, Opts []rest.RouteOpt /*upgrade-time: PathParam codecs, validated once per connection, not per frame*/}` (path-addressed duplex socket — accepted on `SourcePort` (builds `Socket[T,struct{}]`, inbound-only), `SinkPort` (`Socket[struct{},T]`, broadcast-only — WS sibling of SSE), `DuplexPort` (`Socket[In,Out]`); REJECTED on `IOPort`/`LatestPort`/`ToolPort` with `PatternRegisterError{Kind:"socket"}` — per-message req/reply over a socket is ReqReplyPattern territory; builds `Socket[In,Out]{Path, Subprotocols, Route *rest.RouteHandle[struct{},struct{}] /*upgrade validation via rest machinery*/, InFormat format.Format[In], OutFormat format.Format[Out]}`; retrieve with `SocketHandle[In,Out]`; `CustomFormat` same escape-hatch contract — applies to whichever side(s) carry the port's real type, the unused `struct{}` side of a one-directional port is EXEMPT from the type assertion so it never fails); `Opts` carries the *exact* same option vocabulary as `rest.NewRoute`/`events.NewChannel`/`reqreply.NewRoute`/`apimcp.NewTool` (`PathParam`, `QueryParam`, `TopicParam`, `RouteMeta`, `Subscribe`, `Publish`, `ToolMeta`, …) — no new param types. A port declares one `Pattern` per protocol family it binds to (a `ToolPort` over HTTP+MQTT5+MCP declares three). **One construction path — always `Register`, never `ClientHandle`**: internally, each `Pattern` is turned into a handle via `Route`/`Channel`/`Tool.Register(builder)` — the *exact* call a hand-declared route makes — so a `Pattern`-derived handle is indistinguishable from one built by hand with the same builder; adapters cannot tell the difference. `PortOptions.RESTBuilder *rest.Builder` / `EventBuilder *events.Builder` / `ReqReplyBuilder *reqreply.Builder` / `MCPBuilder *apimcp.Builder` — supply your own (with `AddSecurityScheme`/`AddGlobalSecurity`/`rest.WithPathConstraints`/`events.WithTopicConstraints` already configured) to get full parity with a hand-registered route (security schemes, global security, path/topic format constraints, shared spec accumulation); when nil (default), `ports` registers against a private, single-use `Builder` with zero `Info` — same zero-ceremony default, identical `Register` code path. **Handle accessors** (return `(handle, false)` — not an error, not a panic — when the port declared no matching `Pattern`): `RESTHandle[Req,Resp](port) (*rest.RouteHandle[Req,Resp], bool)`, `EventHandle[T](port) (*events.ChannelHandle[T], bool)`, `ReqReplyHandle[Req,Resp](port) (*reqreply.RouteHandle[Req,Resp], bool)`, `MCPHandle[In,Out](port) (*apimcp.ToolHandle[In,Out], bool)`, `FileHandle[T](port) (ports.File[T], bool)`, `SSEHandle[Event](port) (*rest.SSERouteHandle[struct{},Event], bool)`, `SQLMeta(port) (SQLPattern, bool)`, `CacheHandle[T](port) (Cache[T], bool)`, `SocketHandle[In,Out](port) (Socket[In,Out], bool)`. **Spec-from-binding**: `RegisterREST[Req,Resp](b *rest.Builder, port) error` (covers ingest as `RegisterREST[T, struct{}]`) / `RegisterSSE[Event](b, port) error` (SinkPort SSE routes) / `RegisterEvent[T](b, port) error` / `RegisterReqReply[Req,Resp](b, port) error` / `RegisterMCP[In,Out](b, port) error` / `RegisterSocket[In,Out](b *events.Builder, port) error` (renders SocketPattern as an AsyncAPI channel via `events.Builder.AddChannelItem`: channel = path template, {var}s → parameters, Subscribe op = In frames the app RECEIVES, Publish op = Out frames it SENDS; one-directional ports emit only the live direction — struct{} side skipped; declare `events.Server{Protocol:"ws"}` for a complete doc; OpenAPI cannot express WS frames) — replay an already-bound port's `Pattern` against a *different* real spec `Builder` after the fact (only needed when no `Builder` was supplied via `PortOptions` up front — if one was, the route is already registered with it; calling `Register*` again with that *same* builder is a no-op duplicate entry for `rest`/`events` (no dedup) but errors for `reqreply`/`mcp` (`DuplicateRouteError`/"already registered" — these two do dedup); returns `MissingPatternError{Port,Kind}` if the port declared no matching `Pattern`). **`IOParam`** — protocol-agnostic parameter declaration for **handle-less adapters only** (`file.ReadEachAdapter`, `file.DrainWriteFileAdapter` — no `Pattern`/builder of their own): `IOParam{Name, Description, Codec *codex.Codec[string], Required bool}.WithCodec(c)` mirrors `rest.PathParam.WithCodec`; propagated via `WithParams(ctx, params)`/`ParamsFromContext(ctx)`; enforced at runtime via `ValidateParams(params, vars) error` (returns `codex.ValidationErrors`). Handle-backed adapters (REST/events/reqreply/MCP) validate through their `Pattern`-derived handle instead and do not consult `Params`. **`PortOptions`** — full field list: `Patterns []Pattern`, `Params []IOParam`, `Buffer int` (only `SourcePort`/`SinkPort` honor it — `IOPort`/`ToolPort` have no internal channel), `Observer stats.Observer` (a `"port.bind"` `RecordRequest` call + `TraceObserver` span wraps every `Bind`/adapter-Activate call), `RESTBuilder`/`EventBuilder`/`ReqReplyBuilder`/`MCPBuilder`. **Adapter interfaces** (implemented by binding constructors in each adapter package): `SourceAdapter[T]` (`Activate(ctx, dst chan<-T, errs chan<-error)` + `AdapterName() string`), `SinkAdapter[T]` (`Activate(ctx, src Stream[T])` + `AdapterName()`), `IOAdapter[Req,Resp]` (`Transform(ctx, src Stream[Req]) Stream[Resp]` + `AdapterName()`), `ToolAdapter[In,Out]` (`Bind(ctx, fn func(ctx,In)Stream[Out]) error` + `AdapterName()`), `LatestAdapter[T]` (`Serve(ctx, latest func() (T, bool)) error` + `AdapterName()`), `DuplexAdapter[In,Out]` (`Activate(ctx, dst chan<- Framed[In], errs chan<- error, src Stream[Framed[Out]]) error` + `AdapterName()`). **Test helpers**: `ChanSourceAdapter[T](<-chan T)`, `ChanSinkAdapter[T](chan<-T)`, `FuncIOAdapter[Req,Resp](func(ctx,Req)(Resp,error))`. **Errors** (all `slog.LogValuer`; most `Unwrap()`): `PortBindError{Port,Adapter,Err}`, `PortNoAdapterError{Port}`, `PortNoPipelineError{Port}`, `PortNotStartedError{Port,Op}` (no Unwrap — no inner error), `MissingPatternError{Port,Kind}` (`Kind`: `"rest"`/`"event"`/`"reqreply"`/`"mcp"`), `PatternRegisterError{Port,Kind,Err}` (`Kind` now also includes `"file"`/`"cache"`/`"socket"`; wraps the underlying `rest`/`events`/`reqreply`/`apimcp` error — `InvalidPathParamError`, `InvalidTopicError`, `DuplicateRouteError`, etc. — OR a `CustomFormat` type-mismatch (`FormatOptError` from `rest`/`events`/`reqreply`, or a plain error from `resolveFormat`) OR a plain descriptive error for port-type rejections). **Each adapter package has a binding.go**: `mqtt5.SubscribeAdapter/PublishAdapter/CallAdapter/ServeAdapter`, `mqtt.SubscribeAdapter/PublishAdapter` (`SubscribeAdapterOptions.TopicFilter`, when empty, auto-derives an MQTT wildcard filter from `{var}` placeholders in the handle topic — `"sensors/{id}/data"` → `"sensors/+/data"`), `nethttp.IngestAdapter/SSEAdapter/CallAdapter/PollAdapter/DrainCallAdapter/PipelineAdapter/LatestAdapter`, `chi.IngestAdapter/SSEAdapter/PipelineAdapter/IngestSocketAdapter/BroadcastSocketAdapter/DuplexSocketAdapter` (socket variants delegate to adapters/websocket via a constructor-time swapHandler that satisfies websocket.Mux — chi-safe registration, 503 before Activate; naming shims override AdapterName to "chi.*"), `zeromq.SubscribeAdapter/PublishAdapter/CallAdapter/ServeAdapter/LatestAdapter`, `file.ScanAdapter/WatchAdapter/ReadAdapter/ReadEachAdapter/DrainWriteAdapter/DrainWriteFileAdapter/DrainPatchAdapter/DrainPatchEncodedAdapter` (`ReadAdapter[In,Resp]` is the 2-type per-item read pairing with `FilePattern` — file content IS the response; 3-type `ReadEachAdapter[In,T,Resp]` with `combine` stays handle-first for enrichment), `sql.QueryAdapter/QueryEachAdapter/DrainInsertAdapter` (all three default their options' `Table`/`Op` from `ports.SQLMetaFromContext(ctx)` when the explicit fields are empty — explicit values win), `websocket.IngestSocketAdapter/BroadcastSocketAdapter/DuplexSocketAdapter`, `mcpgo.ToolPipelineAdapter/LatestAdapter` (ToolLatestAdapter was REMOVED — LatestPort+LatestAdapter replaces it; no ignored pipeline argument). Stream bridge functions (`SubscribeStream`, `DrainPublish`, `CallStream`, etc.) have been fully removed — use port binding constructors instead. Non-stream adapter functions (`Subscribe`, `Publish`, `Call`, `Serve`, `Handler`) remain fully supported for standalone (non-pipeline) use, including `rest.Route.ClientHandle()`/`events.Channel.ClientHandle()`/`reqreply.Route.ClientHandle()`/`apimcp.Tool.ClientHandle()` for builder-free client-only handles (never called by `ports` internally — see "one construction path" above). REST ingest (`SourcePort`) and SSE (`SinkPort`) are covered by role-aware `RESTPattern` (see above); only `nethttp.DrainCallAdapter` (outbound client POST per item, independent Resp codec) remains handle-first by design. **File I/O** (`ports.File[T]`, moved here from `format` — protocol-agnostic addressing descriptor bound via `FilePattern`, mirrors `Cache[T]`'s placement exactly; only `format.Format[T]` itself stays in `format`): declarative typed file descriptor mirroring `rest.Route`/`events.Channel`'s declare-once pattern; `ports.NewFile(template, format, ...FileOpt) File[T]`; `ports.FilePathParam{Name, Description}` with `.WithCodec(c) ports.FilePathParam` (no `Required` — all template vars must be present); `ports.FileOptions{Observer stats.Observer, Perm os.FileMode}` (type-asserts to `stats.TraceObserver` for distributed tracing spans); methods: `ports.File.Read(vars, opts) (T, error)`, `ports.File.ReadMerged(vars, opts) (T, error)` (decode-merge convenience — merges vars into the SAME decoded value via `codex.DecodeVars` using `File.MergeFields()`; identical to `Read` when no merge fields declared; `adapters/file.ReadEachAdapter`/`ReadAdapter` call this internally), `ports.File.Write(vars, T, opts) error` (use when you already have the decoded value — no re-read), `ports.WriteHandle[T](file, T, opts) error` (single-call convenience — derives vars from T automatically via `codex.EncodeVars(v, file.MergeFields()...)`, mirrors `mqtt5.PublishHandle`; `adapters/file.DrainWriteFileAdapter`'s `varsFor` may be `nil` to use this), `ports.File.Update(vars, func(T) T, opts) error` (re-reads file — use only when you need the latest state; no `UpdateHandle` convenience yet — deferred pending a concrete re-derivation-timing decision, no roadmap doc needed until a use case appears), `ports.File.Patch(vars, patch map[string]any, opts) error` (JSON Merge Patch RFC 7396 — only map-based formats; absent keys preserved), `ports.File.BuildPath(vars) (string, error)`, `ports.File.ValidatePathVars(vars) error`, `ports.File.PathParamSchemas() map[string]schema.Schema`; free function: `ports.PatchEncoded[T, P any](file ports.File[T], vars, patchCodec Codec[P], patch P, opts) error` — typed partial update via a separate patch struct+codec; P must be a struct codec, intermediate must be map[string]any; **field survival**: fields in file codec re-written, fields in BOTH codecs updated, fields in patchCodec only → written (validated by patchCodec, bypasses file codec re-encode), fields in neither codec → dropped; use `ports.File.Patch` for unknown-field-drop semantics; `format.Format.IsPatchable() bool` — true for JSON/YAML/TOML/New, false for Gob/Binary/NewTyped/NewStreamed; typed errors: `ports.FilePathParamError{Name, Value, Err}`, `ports.MissingFilePathVarError{Name}`, `ports.FileReadError{Path, Err}`, `ports.FileDecodeError{Path, Err}`, `ports.FileEncodeError{Path, Err}`, `ports.FileWriteError{Path, Err}`, `ports.FilePatchNotSupportedError{Path}` — all implement `Unwrap()` and `slog.LogValuer`. | `codex`, `stats`, `stream`, `format`, `api/rest`, `api/events`, `api/reqreply`, `api/mcp` |
