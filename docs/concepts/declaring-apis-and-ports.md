...
| Events (`api/events`) | `events.NewChannel[T]("sensors/{id}/readings", ...)` | `events.NewTopic("sensors/{id}/readings", params...)` + `events.NewChannelFromTopic(topic, ...)` | `events.TopicParam` / `events.MergedTopicParam[T]` | ✅ via `events.NewTopicParam[T]` (merges into payload `T`) | ✅ `Channel.WithSubscribe(...)`/`WithPublish(...).Handle(builder)` |
