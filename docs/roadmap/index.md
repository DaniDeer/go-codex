...
| [WebSocket Adapter + Duplex Ports](websocket-adapter.md) | `adapters/websocket`, `ports` | **Design evaluation** | Server-side WS (gorilla/websocket — already indirect dep, behind narrow `Socket` interface): `IngestSocketAdapter` (SourcePort) + `BroadcastSocketAdapter` (SinkPort, WS sibling of SSE) + **new sixth port type `DuplexPort[In,Out]`** (session-tagged `Framed[T]`, targeted replies + broadcast) with **new `SocketPattern`** (path-addressed duplex sockets — RESTPattern roles already taken by ingest/SSE). Streaming-pattern verdict: one universal pattern is WRONG — Redis Streams (key-addressed, at-least-once, XACK) gets its own declaration in redis Phase 3. Client-side WS = Phase 2 |
