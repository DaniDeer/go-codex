...
| [PipePort Composition Hardening](pipe-port-composition-hardening.md) | `ports`, `app` | Gaps 1, 2, 5, 6 shipped | Reworking `examples/sensor-service` with `PipePort`/`Chain`/`ChainStream` surfaced 6 gaps. **Shipped**: observer visibility on the Push/Chain/ChainStream data path (`RecordSubscribe`/`RecordPublish`), `"pipe.chain"` TraceObserver span on stage edges (setup-only), `PipePort.Done()` + `app.App.Supervise` (fixes `Connect`'s fire-and-forget clash with `App.Go`'s blocking contract), `InputPortWithPatterns`/`OutputPortWithPatterns` additive overloads (Pattern-requiring adapters can now bind through PipePort's IO-bridging use case). **Still open/deferred**: `PipelineSpec` blind to IOPort/ToolPort/LatestPort hops hidden inside a `ChainStream` transform closure, and sensor-service's own spec generation still hand-typed — both require the sensor-service rework itself, deferred to a future session. |
