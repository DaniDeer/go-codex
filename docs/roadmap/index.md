...
| [Redis — Phase 2: Pub/Sub](redis-pubsub.md) | `adapters/redis` | Design draft | `SubscribeAdapter` (SourcePort) + `PublishAdapter` (SinkPort) via existing `EventPattern`; new narrow `PubSubCommands` interface + fake; at-most-once semantics documented loudly (offline subscribers LOSE messages — closer to ZeroMQ than MQTT); `PSUBSCRIBE` glob derivation from `{var}` templates; reuses `RecordSubscribe`/`RecordPublish` (no new stats extension). Redis Streams = Phase 3 candidate; still deferred from Phase 1: DelAdapter, CachePattern spec rendering (per-var key codecs now tracked separately, see below) |
| [Cache Key Codecs](cache-key-codecs.md) | `ports`, `adapters/redis` | Design complete | `CachePattern.Opts []CacheOpt` + `ports.CacheKeyParam{Name,Description,Codec}.WithCodec(c)` — per-var codec validation for `Cache[T].BuildKey`'s `{var}` template, mirroring `format.FilePathParam`/`format.File`. New `CacheKeyParamError{Key,Var,Value,Err}`; new `Cache[T].ValidateKeyVars`/`KeySchemas` methods. Zero behavior change when `Opts` is unset |
