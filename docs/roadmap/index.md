...
| [Declarative Var Extraction & Merge](vars-codec-merge.md) | `codex`, `ports`, `api/rest`, `api/events`, `api/reqreply`, `api/internal`, `config` | **Round 1 SHIPPED** — Round 2 (events/reqreply/cache) not yet implemented | Core: `codex.DecodeVars`/`EncodeVars` reuse the EXACT `Field[T,F]`/`RequiredField`/`OptionalField`/`DefaultField` declarations already used for `Struct[T]` to decode/encode a `map[string]string` (path/topic/header/query/cookie/filename vars) instead of a JSON object. `ports.File[T].MatchPath` — missing inverse of `BuildPath`, mirrors `mqtt.TopicVarsFromMessage`. Additive per-boundary constructors SHIPPED for REST + File: `rest.NewPathParam[T]`/`NewRequiredQueryParam[T]`/etc., `ports.NewFilePathParam[T]` — ONE call declares both the spec Param and a merge field; `RouteHandle.DecodeMerged` + `nethttp`/`chi` adapter integration close the loop fully for REST, demonstrated in `examples/adapters-nethttp`/`adapters-chi`. Considered and rejected making the 7 existing Param types themselves generic (verbosity regression for the common validate-only case, no extra capability). Round 2 (documented, not yet shipped): `events.NewTopicParam[T]`, `reqreply.NewTopicParam[T]`, `ports.NewCacheKeyParam[T]`, `config.md` env-var-overrides doc update |
