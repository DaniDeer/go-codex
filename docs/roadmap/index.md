...
| [Wildcard/Glob Path Template Segments](path-template-wildcards.md) | `ports` | Idea only | Follow-on to the now-SHIPPED Directory Listing Port (`ports.Dir`/`DirPathParam`, see `docs/features/ports.md`) — sketches adding a wildcard/glob segment type (e.g. `*`/`**`) to BOTH `FilePathParam` (`ports.File`) and `DirPathParam` (`ports.Dir`) symmetrically, since neither supports anything beyond named `{var}` placeholders today. Not designed: open questions include single- vs. multi-segment wildcard scope, whether it needs a dedicated param type vs. extending the existing ones, and how `BuildPath` (which has no meaning for a wildcard) should behave for a wildcarded template. |
