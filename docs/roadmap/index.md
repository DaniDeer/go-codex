...
| [Inside-Out Pipeline Wiring](inside-out-pipeline-wiring.md) | `ports`, all adapters | **✅ Phase 1 implemented** | Protocol-agnostic `SourcePort[T]` / `SinkPort[T]` / `IOPort[Req,Resp]` + `IOParam`; adapter bindings for all transports; fan-in/fan-out; zero adapter imports in domain code |
