...
| [Ports — Post-Phase-6 Gaps](ports-post-phase6-gaps.md) | `ports`, `stream`, `forge`, adapters | **Gap analysis + sketches** | What remains around the shipped inside-out ports story (four port types, six `Pattern` kinds — now documented in [Ports feature](../features/ports.md)/[guide](../guides/ports.md)): `LatestPort[T]` cache port (G1, high), `SinkPort.Push` request-scoped submission (G2, high), REST ingest/SSE pattern support (G3), `forge.App` lifecycle (G4), topology port-step kind (G5), `stream.Map` (G6) |
