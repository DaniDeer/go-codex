...
| [Error Path Ergonomics Unification](error-path-ergonomics.md) | `api/rest`, adapters, ports | Design complete | Additive unification of error-path declaration across plain HTTP handlers, pipeline handlers, ports-wired patterns, and req/reply adapter families. Keeps `ErrorHandler`/`OnError` escape hatches while introducing a consistent typed declaration model that mirrors happy-path req/resp ergonomics. |
