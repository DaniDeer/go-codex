...
| [SSE & WebSocket Merge-Field Gaps](sse-websocket-merge-field-gaps.md) | `api/rest` (SSE), `adapters/nethttp`, `adapters/chi`, `ports.DuplexPort`, `adapters/websocket` | Design complete | Consolidates two structurally-identical, previously-deferred open questions: `rest.SSERouteHandle` has zero merge support for the pushed `Event` type, and `adapters/websocket`'s per-session upgrade vars (`Hub.SessionInfo`) never merge into per-message frame payloads. Both are long-lived CONNECTIONS carrying many messages from one initial request — a genuinely different shape ("derive vars once, apply many times") from every other shipped merge site ("derive once, apply once"). Central open question: is repeating one connection's vars into every message actually useful, or does the existing closure/`SessionInfo`-based escape hatch already cover the need? No concrete use case has appeared for either — design only, not yet implemented. (`ports.File`/`ports.Cache` gained the same "one struct, one call" convenience `api/rest`/`api/events`/`api/reqreply` already had — `File.ReadMerged`/`ports.WriteHandle`, `redis.GetMerged`/`redis.SetHandle` — fully shipped, see `.github/skills/review-go-codex/references/history.md`'s Round 63) |
