...
| [Partial/Patch Struct Codec](partial-struct-codec.md) | `codex` | Design draft | Generalizes "patch an existing struct" (only SOME fields set, sparse encode/decode) into reusable core primitives — `PartialField`/`PartialStruct` — additive, zero changes to `Struct`/`FieldCodec`/`ports.PatchEncoded`. Each patch field's validation is INHERITED by reusing the base struct's own existing `Codec[F]` unchanged; recursive nesting auto-collapses empty nested sub-objects. NOT reflection-based auto-derivation (Go generics can't enumerate an arbitrary struct's fields) — the patch struct's shape is still hand-declared, same as `codex.Struct[T]` itself requires. Flagship consumer: refactor `examples/go-edge-models`'s hand-rolled `modulepatch.ModuleFieldsPatchCodec` to use it. |
