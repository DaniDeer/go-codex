...
| [File & Cache Merge-Field Gaps](file-cache-merge-field-gaps.md) | `ports/file.go`, `adapters/file`, `ports/cache.go`, `adapters/redis` | Design complete | A `review-go-codex` audit of every adapter/port against the merge-field convenience pattern found `ports.File`/`ports.Cache` have the declare-once constructors (`NewFilePathParam`/`NewCacheKeyParam`) and `MergeFields()` accessors, but NEVER got the single-call convenience wrapper (`ReadMerged`/`WriteHandle`/`GetMerged`/`SetHandle`) — `adapters/file`'s `ReadAdapter`/`DrainWriteFileAdapter` and `adapters/redis`'s `SetAdapter`/`DrainSetAdapter` require a mandatory hand-written vars/key closure even when the item type already declares its merge fields. Also flags a related, lower-priority open question for `adapters/websocket`'s per-connection path vars (same shape as SSE's already-deferred question) and a stale row in the `review-go-codex` checklist |
