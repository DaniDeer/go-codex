...
| [Partial/Patch Struct Codec](partial-struct-codec.md) | `codex` | Design refined (v2) | Generalizes "patch an existing struct" (only SOME fields set, sparse encode/decode) into reusable core primitives — `PartialField`/`PartialStruct` — additive, zero changes to `Struct`/`FieldCodec`/`ports.PatchEncoded`. Each patch field's validation is INHERITED by reusing the base struct's own existing `Codec[F]` unchanged. Nesting is NOT a separate mechanism — it falls out for free from ordinary `Codec[T]` composability (no auto-collapse heuristic needed). NOT reflection-based auto-derivation (Go generics can't enumerate an arbitrary struct's fields) — the patch struct's shape is still hand-declared, same as `codex.Struct[T]` itself requires. Flagship consumer: refactor `examples/go-edge-models`'s hand-rolled `modulepatch.ModuleFieldsPatchCodec` to use it (breaking-reshapes `ModuleFieldsPatch` for full field-pointer uniformity — acceptable, single consumer). |
| [`PartialFrom` — Diff Two Structs Into a Patch](partial-struct-diff.md) | `codex` | Idea only | Follow-on to Partial/Patch Struct Codec — auto-derive a patch by diffing a base/previous `T` against an updated `T`, populating only the CHANGED fields. Not scoped, no use case yet; the open question is per-field equality without reflection (many real `F` types aren't `comparable` in Go's strict sense). |
