...
| [Declarative Var Extraction & Merge](vars-codec-merge.md) | `codex`, `ports`, `api/rest`, `api/events`, `api/reqreply`, `api/internal`, `config` | **Rounds 1, 3, 4 SHIPPED** — Phase 2 (events/reqreply/cache) designed, not yet implemented | `codex.DecodeVars`/`EncodeVars` reuse the EXACT `Field[T,F]`/`RequiredField`/`OptionalField`/`DefaultField` declarations already used for `Struct[T]` to decode/encode a `map[string]string` (path/topic/header/query/cookie/filename vars). REST is now the full reference implementation of the "one struct, one call" mandate — request AND response merge, role-aware encode accessors, `nethttp.CallHandle` single-call convenience, nested struct composition, and non-JSON body formats (Gob/Binary/custom via `format.NewTyped`) all shipped and demonstrated (`examples/adapters-nethttp`, `adapters-chi`, `adapters-nethttp-client`, `rest-nested-binary`). This pattern is now a MANDATORY design contract for every `api/*` builder-backed boundary (see `.github/instructions/go-codex.instructions.md`, `add-a-new-adapter` skill's Step 5b). Phase 2 (fully designed in the roadmap doc, not yet implemented): `events.NewTopicParam[T]`/`ChannelHandle.DecodeMerged`/`mqtt5.PublishHandle`, `reqreply.NewTopicParam[T]` + adapter-specific single-call convenience, `ports.NewCacheKeyParam[T]` — plus a newly-found prerequisite gap (`adapters/mqtt5` has no `TopicVarsFromMessage` equivalent, unlike the older `adapters/mqtt` v3 package) |
