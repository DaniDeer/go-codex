...
| [Ports & Pipeline Ergonomics](ports-pipeline-ergonomics.md) | `ports`, `forge` | Design complete | `examples/sensor-service` proved the declarative ports/pipeline model works but is verbose in practice: a 4-line handle-derivation "ok-check dance" repeats 6x; `PipePort`'s IO-bridging overloads are measurably MORE verbose than plain `SourcePort`/`SinkPort` for the same job (confirmed via diff); `forge.Function` wrapping costs 9 lines for 1 line of logic. Central proposal (breaking changes OK, no legacy path kept — all open design decisions resolved): a **Pattern-plugin model** — `Pattern`s declared as standalone values, ports declared with no patterns baked in, `PortOptions.Patterns` REMOVED entirely, a new `PluginXxxPattern` call at wiring time both registers the Pattern AND returns the typed handle in one step (applies uniformly across all 6 port types); thin `NewRestPort`/`NewReqReplyPort`/`NewMCPPort`/`NewSQLPort` convenience constructors wrapping `IOPort`/`ToolPort` + one `Plugin` call for the single-Pattern common case (keeping `IOPort` genuinely backend-agnostic); scoping `PipePort` back to computation-only; generalizing `Chain`/`ChainStream` to accept boundary ports directly (`SourcePort -> Chain -> PipePort -> ChainStream -> SinkPort` as one visible declaration); guidance to skip `forge.Function` for ungoverned pipeline glue; plus a smaller additive `gstream.LogOnError` helper found while cross-checking the shipped "one-struct-one-call" precedent. |
