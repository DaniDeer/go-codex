...
| [Stream — Phase 4 (FlatMap, GroupBy)](stream-phase4.md) | `stream` | Awaiting use case | FlatMap sub-stream variant (goroutine pool; main IO driver now covered by `ports.IOPort` + 1→N adapters) and GroupBy (per-key sub-streams) — deferred until a concrete driver appears. CombineLatest5+ resolved via nested composition (documented in the stream guide) |
