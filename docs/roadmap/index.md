...
| [Ports — Post-Phase-6 Gaps](ports-post-phase6-gaps.md) | `ports`, `stream`, `app`, adapters | **All phases implemented** | Shipped: `LatestPort[T]` cache port (G1, fifth port type — `nethttp`/`zeromq`/`mcpgo` `LatestAdapter`s; `ToolLatestAdapter` removed), `SinkPort.Push`/`Start`/`Close` (G2), role-aware `RESTPattern` for HTTP ingest + SSE (G3 — `SSEHandle`/`RegisterSSE`), topology `StepKindPort`/`WithPort` (G5), `stream.Map` (G6), NEW top-level `app` lifecycle package (G4 — [feature page](../features/app.md)). Only deliberately deferred G7 (dynamic rebinding) remains; kept as design record |
