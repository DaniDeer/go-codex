...
| [LLM Integration](llm-integration.md) | `api/llm`, `adapters/openai`, `render/openaitools` | Design complete | The other direction from the already-shipped MCP server: go-codex CALLING an LLM. New `api/llm.Call[Req,Resp]` (declare→register→handle, mirrors `api/reqreply`) carries a system prompt (string or Markdown file) + input/output codecs; `adapters/openai.CallAdapter` implements `ports.IOAdapter[Req,Resp]` against any OpenAI-compatible Chat Completions endpoint using strict `response_format:json_schema`, re-validated locally via `codex.Refine`, with a bounded retry-on-invalid-completion loop. `render/openaitools` renders existing `mcp.ToolSpec`/new `llm.CallSpec` into the OpenAI `tools` JSON array — zero new declaration surface for that direction. Stdlib-only (`net/http`), no SDK dependency. Agent2Agent (A2A) protocol, streaming, multi-turn tool loops, and multimodal input are documented as Phase 2+; LangChain needs no new code (already MCP/tool-array compatible). |
| [Vector Store Adapter (RAG retrieval)](vector-store-adapter.md) | `adapters/vectorstore`, `adapters/openai` (embeddings) | Awaiting use case | Placeholder for RAG's retrieval half — a narrow `VectorStore` interface (mirrors `adapters/redis.Commands`) for native vector DBs (Qdrant/Weaviate/Pinecone/Redis-search), plus the `adapters/openai` embeddings endpoint `llm-integration.md` deferred. Notes what already works with ZERO new code today: pgvector similarity search via the existing generic `adapters/sql.QueryEachAdapter`, and embedding vectors via `codex.SliceOf(codex.Float32())`. Not yet designed in detail — captured as a reminder pending a concrete driving use case. |
