...
| [PipePort Composition Hardening](pipe-port-composition-hardening.md) | `ports`, `app` | Design draft | 6 gaps identified reworking `examples/sensor-service` with `PipePort`/`Chain`/`ChainStream`: zero observer visibility on the Push/Chain/ChainStream data path, no TraceObserver span on stage edges, `PipelineSpec` (informal YAML docs only — verified real OpenAPI/AsyncAPI specs are unaffected) blind to IOPort/ToolPort/LatestPort hops hidden inside a `ChainStream` transform, sensor-service's own spec generation still hand-typed, `PipePort.Connect`'s fire-and-forget shape clashing with `app.App.Go`'s blocking-function contract, and `InputPort`/`OutputPort` never forwarding `Patterns` (blocking Pattern-requiring adapters from PipePort's IO-bridging use case). Fix directions proposed per gap; none implemented yet. |
