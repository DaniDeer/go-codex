...
| [Inside-Out Pipeline Wiring](inside-out-pipeline-wiring.md) | `forge`, all adapters | **Design evaluation** | Protocol-agnostic `SourcePort[T]` / `SinkPort[T]` in the pipeline; adapter bindings wired in main.go; fan-in sources, fan-out sinks; zero adapter imports in domain code |
