...
| [SSE & WebSocket Merge Hardening](sse-websocket-merge-hardening.md) | `api/rest` (SSE), `adapters/nethttp`, `adapters/chi`, `ports.DuplexPort`, `adapters/websocket`, docs/examples | Design draft | SSE/WebSocket connection-level merge shipped (`NewRequiredSSEEventParam`, `SocketPattern.InOpts/OutOpts`, adapter auto-merge). This follow-up tracks remaining hardening only: nested/non-JSON integration coverage and runnable docs/examples so "one struct, one call" for long-lived boundaries is documented as deeply as REST/events/reqreply. |
