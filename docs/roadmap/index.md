...
| [WebSocket — Phase 2: Client-Side + chi](websocket-phase2.md) | `adapters/websocket` | Deferred | Phase 1 SHIPPED (server-side `IngestSocketAdapter`/`BroadcastSocketAdapter`/`DuplexSocketAdapter`, sixth port type `DuplexPort[In,Out]` with session-tagged `Framed[T]`, `SocketPattern`). Remaining: client-side dial adapters (reconnect-gap visibility open), chi variants (swap-handler pattern), `ConnectionObserver` extension, dynamic subprotocol negotiation, AsyncAPI spec via ws bindings (`RegisterSocket` — OpenAPI can't express WS). Will-not-implement: MQTT-over-WS (client transport option), universal StreamPattern |
