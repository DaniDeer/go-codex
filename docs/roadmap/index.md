...
| [File & Cache Merge-Field Gaps](file-cache-merge-field-gaps.md) | `ports/file.go`, `adapters/file`, `ports/cache.go`, `adapters/redis` | G1/G2/G4 shipped; G3 deferred | `ports.File`/`ports.Cache` gained the single-call convenience wrapper (`File.ReadMerged`/`ports.WriteHandle`, `redis.GetMerged`/`redis.SetHandle`) that REST/events/reqreply already had — `adapters/file`'s `DrainWriteFileAdapter` and `adapters/redis`'s `SetAdapter`/`DrainSetAdapter` now derive vars per-item automatically when their closure argument is `nil`; `ReadEachAdapter`/`ReadAdapter`/`GetAdapter` merge already-known vars into decoded values automatically. Also found and fixed a real, pre-existing bug: both `CachePattern` build paths silently dropped `NewCacheKeyParam`-registered merge fields entirely (fixed by delegating to `NewCache`, mirroring `FilePattern`'s existing delegation to `NewFile`). `adapters/websocket`'s per-connection path-var question remains deferred alongside SSE's equivalent open question |
