...
| [Ports — Post-Phase-6 Gaps](ports-post-phase6-gaps.md) | `ports`, `stream`, `forge`, adapters | **Reviewed and phased** | What remains around the shipped inside-out ports story (four port types, six `Pattern` kinds — now documented in [Ports feature](../features/ports.md)/[guide](../guides/ports.md)). Phased plan: **A** `SinkPort.Push` (G2) then `LatestPort[T]` (G1) — design complete, blockers resolved; **B** topology port step (G5) + `stream.Map` (G6); **C** REST ingest/SSE patterns (G3); **D** `forge.App` (G4) |
