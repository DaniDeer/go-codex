...
| [Merge-Field Remaining Gaps](merge-field-remaining-gaps.md) | `api/rest` (SSE), `adapters/mqtt`, `adapters/mqtt5`, `adapters/zeromq`, `ports`, `api/mcp`, `adapters/mcpgo` | Design complete | Post-shipment review of the merge-field-port-adapter-gaps round (all four of its gaps shipped) found two carried-forward deferred items still open — `rest.SSERouteHandle` has zero merge support (needs a design decision on per-connection vs. per-event merge before implementing), and FOUR (not three) near-identical topic/path template-matching implementations (`mqtt`, `mqtt5`, `zeromq`, `ports/file.go`) exist because none can import the `api/*`-only `api/internal.MatchTemplate`. Also found a pre-existing, unrelated data race in `adapters/mqtt`'s test mock (trivial mutex fix), and completed the first-ever `api/mcp` assessment: Resources have ZERO automatic URI-var extraction/validation today (more basic than a merge-field gap — `ResourceHandle.ValidateURIVars` is never called by `adapters/mcpgo`), while Prompts auto-validate but hand the app a raw map instead of a merged struct |
