...
| [Security Scheme Parity — events/reqreply/mcp](events-reqreply-mcp-security-scheme.md) | `api/events`, `api/reqreply`, `api/mcp` | Design draft | Follow-up to the shipped `api/rest` security symmetry: investigates what parity means per layer — `api/events` has builder-level `AddSecurityScheme` today (Phase 1: migrate to channel-level `WithSecurityScheme`; Phase 2: net-new publish-side `CredentialFunc` for mqtt/mqtt5/zeromq, no existing mechanism); `api/reqreply` has NOTHING (Phase 3: full net-new security feature); `api/mcp` has NOTHING BY DESIGN (Phase 4: **blocked on explicit sign-off** — reverses a documented "MCP security is out of scope" decision, not a routine gap). |
| [Async/Refreshable Credential Caching](credential-caching.md) | `adapters/nethttp` | Design draft | A generic `CredentialFunc` wrapper (`nethttp.NewCachingCredentialFunc`) adding TTL-based caching and retry-on-401 semantics, orthogonal to the format-validation-only scope of the shipped security-scheme-symmetry feature. Sketches API surface, concurrency (stampede prevention), and observer integration; not yet fully designed — captured pending a concrete driving use case. |
