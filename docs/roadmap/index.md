...
| [Route-Only Security Scheme Declaration](security-scheme-symmetry.md) | `api/rest`, `adapters/nethttp` | Design complete | Breaking change: removes `Builder.AddSecurityScheme`/`Builder.securitySchemes` entirely — new route-level `rest.WithSecurityScheme` RouteOpt becomes the SOLE way to declare a security scheme+codec, consumed identically by `Route.Register` (server) and `Route.ClientHandle` (client). Adds a symmetric client-side credential-format check to `nethttp.Call` (reuses `validateSecurityCredentials`/`extractCredential`/`rest.SecurityCredentialError`/`stats.SecurityObserver` verbatim — zero new error/observer types). `OpenAPISpec()` aggregates schemes from routes instead of the builder. Explicitly does NOT enforce "Security declared ⇒ CredentialFunc required" (preserves the intentional, documented "unauthenticated request to a secured route → server 401" pattern). Scoped to `api/rest` only — `api/events`/`api/reqreply`/`api/mcp` untouched. |
