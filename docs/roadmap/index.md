...
| [Connect-Level Credential Codec](mqtt-connect-credential-scheme.md) | `adapters/mqtt`, `adapters/mqtt5` | Design draft | Codec-validated CONNECT-time (username/password) credential, protocol-symmetric across MQTT 3.1.1 + MQTT5 (unlike the shipped message-level model, which is MQTT5-only). A `SecuredClient` wrapper validates ONCE, synchronously, at construction — struct-embeds the underlying `MQTTClient`/`pahomqtt.Client` for zero-code-change call sites afterward. Documents a rejected alternative (global per-client memoization) in favor of this simpler, stateless wrapper design. |
