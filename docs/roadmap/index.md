...
| [Pipeline Segmentation — PipePort](pipeline-segmentation-ports.md) | `ports` | SHIPPED | `PipePort[T]` — explicit patch panel connecting named SourcePort inputs to named SinkPort outputs with fan-in/fan-out. Reuses existing adapter interfaces; zero new stats extensions. |
