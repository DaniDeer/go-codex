...
| [Ports — Post-Phase-6 Gaps](ports-post-phase6-gaps.md) | `ports`, `stream`, `forge`, adapters | **Phases A+B implemented** | Phases A+B shipped: `LatestPort[T]` cache port (G1, fifth port type — `nethttp`/`zeromq`/`mcpgo` `LatestAdapter`s; `ToolLatestAdapter` removed), `SinkPort.Push`/`Start`/`Close` (G2), topology `StepKindPort`/`WithPort` (G5), `stream.Map` (G6). Remaining: **C** REST ingest/SSE patterns (G3); **D** `forge.App` (G4, needs design pass); G7 deferred |
