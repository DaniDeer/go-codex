...
| [File → Ports Migration](file-to-ports-migration.md) | `format`, `ports`, `adapters/file` | Design evaluation | Move `format.File`/`FilePathParam`/`FileOpt`/7 error types/`PatchEncoded` into `ports`, mirroring `ports.Cache[T]`'s placement exactly (both "have a" `format.Format[T]` but ARE declarative descriptors bound via a `Pattern` to one adapter family). Breaking change — full symbol rename map, ~15 `.go` + ~20 `.md` files impacted. Do not start without explicit go-ahead |
| [Config Package](config-package.md) | new `config` package | Design evaluation | Extract `format.FromEnv`/`FromEnvVar`/`EnvVarError` into a dedicated `config` package — NOT `ports` (no `Pattern`/adapter relationship exists, unlike `File`). Much smaller blast radius (~6 `.go` + ~8 `.md` files) than the companion File migration |
