...
| [MCP Tool ↔ REST Client Bridge](mcp-rest-tool-bridge.md) | `adapters/mcprest` (new) | Design complete | A small bridging package connecting `adapters/nethttp`'s REST client (`CallHandle`) to `adapters/mcpgo`'s MCP tool handler shape — `ToolHandler`/`MappedToolHandler` let an already-declared `rest.Route` become an MCP tool with one function, with optional mapper functions when the tool's In/Out shape differs from the route's wire Req/Resp. FIXED `CredentialFunc` per tool (matches every other client-adapter binding); `DefaultErrorPatterns()` surfaces REST client error detail (status/body) to the LLM via the existing `apimcp.ErrorPattern` mechanism. Driving use case: `examples/go-edge-models/docker/registry`. |
