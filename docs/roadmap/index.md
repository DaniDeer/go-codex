...
| [PipePort Composition Hardening](pipe-port-composition-hardening.md) | `ports`, `app` | Design draft | 5 gaps identified reworking `examples/sensor-service` with `PipePort`/`Chain`/`ChainStream`: zero observer visibility on the Push/Chain/ChainStream data path, no TraceObserver span on stage edges, `PipelineSpec` blind to IOPort/ToolPort/LatestPort hops hidden inside a `ChainStream` transform, sensor-service's own spec generation still hand-typed, and `PipePort.Connect`'s fire-and-forget shape clashing with `app.App.Go`'s blocking-function contract. Fix directions proposed per gap; none implemented yet. |
