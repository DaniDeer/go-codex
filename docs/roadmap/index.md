...
| [Redis — Phase 2: Pub/Sub](redis-pubsub.md) | `adapters/redis` | Design draft | `SubscribeAdapter` (SourcePort) + `PublishAdapter` (SinkPort) via existing `EventPattern`; new narrow `PubSubCommands` interface + fake; at-most-once semantics documented loudly (offline subscribers LOSE messages — closer to ZeroMQ than MQTT); `PSUBSCRIBE` glob derivation from `{var}` templates; reuses `RecordSubscribe`/`RecordPublish` (no new stats extension). Redis Streams = Phase 3 candidate; still deferred from Phase 1: DelAdapter, per-var key codecs, CachePattern spec rendering |
| [CustomFormat — Binary Escape Hatch for Pattern](pattern-custom-format.md) | `ports` | **Design complete** | `CustomFormat any` field on `FilePattern`/`CachePattern`/`SocketPattern` accepting a pre-built `format.Format[T]` (Gob, `format.Binary` for PNG/PDF, or any future format) — closes the gap where these 3 patterns are locked to `FileFormatKind{JSON,YAML,TOML}` while REST/Event/ReqReply patterns already accept any format via `.WithFormats()`. Type-asserted at build time; mismatch → existing `PatternRegisterError` (no new error type). No dedicated `FileFormatGob` enum value — Gob is architecturally different from J/Y/T (no map[string]any intermediate) and belongs in the general escape hatch, not the closed enum |
