...
| [Inside-Out Pipeline Wiring — Phase 2](inside-out-pipeline-wiring.md) | `ports`, `adapters/chi`, `adapters/mcpgo` | **Phase 2 design** | Phase 1 complete. Phase 2: `ports.ToolPort[In,Out]` (server-side req/resp port); `adapters/chi/binding.go`; `adapters/mcpgo/binding.go`; server-side adapters for nethttp/zeromq/mqtt5 (`PipelineAdapter`, `ServeAdapter`) |
