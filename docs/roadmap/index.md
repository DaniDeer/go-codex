...
| [Declarative Var Extraction & Merge](vars-codec-merge.md) | `codex`, `ports`, `api/rest`, `api/events`, `api/reqreply`, `api/internal`, `config` | Design complete | Core: `codex.DecodeVars`/`EncodeVars` reuse the EXACT `Field[T,F]`/`RequiredField`/`OptionalField`/`DefaultField` declarations already used for `Struct[T]` to decode/encode a `map[string]string` (path/topic/header/query/cookie/filename vars) instead of a JSON object. New `ports.File[T].MatchPath` — missing inverse of `BuildPath`, mirrors `mqtt.TopicVarsFromMessage`; shares new `api/internal.MatchTemplate`. Elevated (breaking changes permitted, but design stays additive): new per-boundary constructors (`rest.NewPathParam[T]`, `NewRequiredQueryParam[T]`/etc., `events.NewTopicParam[T]`, `reqreply.NewTopicParam[T]`, `ports.NewFilePathParam[T]`/`NewCacheKeyParam[T]`) eliminate the "declare twice" gap — ONE call declares both the spec Param and a merge field; `RouteHandle.DecodeMerged` + `nethttp`/`chi` adapter integration close the loop fully for REST. Considered and rejected making the 7 existing Param types themselves generic (verbosity regression for the common validate-only case, no extra capability). `config.FromEnv`/`FromEnvVar`'s "env var overrides" recipe benefits from `codex.DecodeVars` directly, no new config-specific code — `adapters/sql` does NOT benefit (sqlc rows have no template-string boundary to parse) |
