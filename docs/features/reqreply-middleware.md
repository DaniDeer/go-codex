...
- [Feature: Observer Pattern](observer.md#apireqreplys-general-purpose-middleware-as-a-declarative-observer-wrapper) — the DIFFERENT, general-purpose (unpaired) `.HandleMW(nil, fn)`/`.ClientMW(nil, fn)` mechanism this SAME package exposes, used for observability rather than codec-backed enrichment — see Demo 11 (`demo_observer_middleware.go`)
