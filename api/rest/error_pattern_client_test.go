...
// adapter-independent — they touch ONLY the ErrorPatternValuer interface,
// never any concrete adapter type. localErrorPatternValue is a minimal,
// package-local test type (NOT nethttp.ErrorPatternResponse or any other
// adapter's type) proving these functions work against ANY value
// implementing ErrorPatternValuer, with zero adapter dependency at all —
// see adapters/nethttp/error_pattern_client_test.go for the complementary
// real-round-trip regression coverage proving nethttp's own
...
func TestErrorPatternAs_NonMatchingError_ReturnsFalse(t *testing.T) {
	_, ok := ErrorPatternAs[errPayload](PathParamError{Name: "id"})
	if ok {
		t.Error("want ErrorPatternAs to return false for an unrelated error type")
	}
}
