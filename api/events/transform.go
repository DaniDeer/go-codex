...
type MiddlewareHandler struct {
	...
	// SAME raw topic var map the channel's own DecodeMerged uses —
	// independent of T, using ONLY this middleware's own topic merge-field
	// declarations and the middleware's own InCodec. Returns the decoded
	// In boxed as `any` (its concrete type is recovered by the adapter via
	// reflection, mirroring how [middleware.ServerImplementation.Fn] is
	// already reflect-called).
	DecodeIn func(topicVars map[string]string) (any, error)
...
type ClientMiddlewareHandler struct {
	...
	// EncodeOut derives outgoing topic-var values from the Out value
	// returned by Fn, using ONLY this middleware's own publish-topic
	// merge-field declarations and the middleware's own OutCodec.
	EncodeOut func(out any) (topicVars map[string]string, err error)
...
func buildDecodeIn[In, Out any](mw Middleware[In, Out]) func(topicVars map[string]string) (any, error) {
	fields := mw.topicMergeFieldsIn
	return func(topicVars map[string]string) (any, error) {
		var in In
		if len(fields) > 0 {
			if err := codex.DecodeVars(&in, topicVars, fields...); err != nil {
...
// [buildAgnosticClientMiddlewareHandler] (channel-AGNOSTIC).
func buildEncodeOut[In, Out any](mw Middleware[In, Out]) func(outAny any) (map[string]string, error) {
	fields := mw.topicMergeFieldsOut
	return func(outAny any) (map[string]string, error) {
		out, _ := outAny.(Out)
		...
		return codex.EncodeVars(out, fields...)
	}
}
