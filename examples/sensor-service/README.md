...
- **Pipeline spec** (`ports.PipelineSpec("Sensor Service MQTT Pipeline", "1.0.0", ioports.Sensors, pipeline.Params, pipeline.Saved, ioports.Alerts)`)
  — the MQTT pipeline shape derived directly from the real port/`PipePort`
  wiring, not hand-typed: pipe/port names, buffer sizes, bound adapter
  identities, and every `Chain`/`ChainStream` edge (including each
  transform's real Go function identity via reflection — e.g.
  `buildInsertParams` for the pure map, honestly closure-opaque for the
  SQL-persistence and filter+alert transforms). No separate
  `pipeline.Topology` function to keep in sync with the code — see
  [`docs/features/ports.md`](../../docs/features/ports.md#pipeport).
