...
- Three of the six path patterns (`baselinePathPattern`/`useCaseEntryShape`/`deviceEntryShape`) have nothing to substitute (a single fixed path, or a filename SHAPE `ports.EntryPattern` EXTRACTS a var FROM, not one this package substitutes into) and stay a plain `codex.Const[string]` — validated ONCE at package init via `codex.MustConst` (an empty/malformed pattern panics immediately, not silently, at first use), read via `String()` only. The other three (`useCasePathPattern`/`deviceDirPathPattern`/`deviceFilePathPattern` — the ones with `{var}` placeholders a CALLER substitutes) are a `codex.Template[T]` built via `codex.NewTemplate` + `codex.IdentityField`/`RequiredField` — `codex.Template.Build` substitutes an already-validated `Name`/`DeviceID` into the pattern directly, with zero `ports.File` I/O involved. `ports.NewFile`/`NewDir`'s own `{var}` substitution is unaffected — it still consumes each pattern's raw, unsubstituted text via `String()`. See `docs/concepts/codec.md`'s "Getter/Setter: validated value containers built on Codec[T]" and "`Template[T]`" subsections for the underlying `codex.Const[T]`/`Getter[T]`/`Template[T]` design.
