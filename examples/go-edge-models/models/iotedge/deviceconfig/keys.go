...
// manifesttemplate/keys.go's role one package over, but scoped strictly
// to what is UNIQUE to this package's own wire shape — the shared
// top-level wire-key names ([manifesttemplate.ModulesContentKey]/
// [manifesttemplate.EdgeAgentKey]/[manifesttemplate.EdgeHubKey]) and the
// dotted-key PREFIX ([manifesttemplate.ModuleKeyPrefix]) both stay
// imported from manifesttemplate/keys.go UNCHANGED, not re-declared or
// re-exported here — manifesttemplate/keys.go remains the single source
// of truth for THAT shared vocabulary (see deviceconfig/doc.go).

// EdgeAgentPatchTemplate is the MQTT-style dotted-key template every
// Patch.EdgeAgent wire key must match: a [manifesttemplate.ModuleKeyPrefix]-
...
const EdgeAgentPatchTemplate = manifesttemplate.ModuleKeyPrefix + "{moduleName}.#"
...
// key must start with manifesttemplate.ModuleKeyPrefix, the immediately
// following segment ("{moduleName}") must be a valid slug — the SAME
// constraint manifesttemplate.ModuleNameCodec applies to a whole module
