...
|---|---|---|
| [`models/iotedge/manifesttemplate/`](models/iotedge/manifesttemplate) | The PURE WIRE FORMAT of an Azure IoT-Edge deployment manifest — Go package `manifesttemplate`, nested under `models/iotedge` (directory name matches the package name exactly, no dash). `ModuleConfig`, `ModuleSettings`, `EnvVars` (string/int/float union), and `ModuleName`/`Modules`/`DeploymentManifest` — dotted-key extraction (`"properties.desired.modules.<name>"`) via `codex.Map`. `FlattenEnvVars(vars EnvVars) docker.Env` is a ONE-DIRECTION mapper (manifesttemplate → docker only) that formats each typed string/int/float `EnvVarValue` as a flat "KEY=VALUE" `docker.EnvVar` — no reverse mapper (would require guessing the original value's type from a flat string). Plain structs + `codex.Codec[T]` ONLY — no file I/O, no `ports.File`/`ports.Dir`, no derived/reduced views; see the parent `models/iotedge` for everything built on top. | `models/docker` (for `ModuleSettings.CreateOptions` and `FlattenEnvVars`'s return type) |
| [`models/iotedge/deviceconfig/`](models/iotedge/deviceconfig) | The PURE WIRE FORMAT of one device's device-specific config file — Go package `deviceconfig`, nested under `models/iotedge`. `Manifest`/`ManifestCodec` only (`DisplayName`/`Enabled`) — a small, minimal illustrative override document. Same rule as `manifesttemplate`: plain struct + codec only; see the sibling `models/iotedge/usecase`'s `device.go` for the templated file port built on top. | *(none)* |
| [`models/iotedge/usecase/`](models/iotedge/usecase) | Everything DERIVED/CONSTRUCTED for a "use case" AND the devices nested under it — never wire formats themselves. `Name`/`DeviceID` (named string types, validated via `NameCodec`/`DeviceIDCodec`, constructed via `NewName`/`NewDeviceID`) are this package's typed vocabulary, used throughout instead of bare `string`. The full use-case model/composition — `NewFile(basePath) ports.File[manifesttemplate.DeploymentManifest]` (a declared, TEMPLATED file port over `"{basePath}/usecases/{usecase_name}.json"`), `NewDir`/`ListNames` (returns `[]Name`), and the domain composition `UseCase`/`Read`/`Write` (which also assembles every nested `DeviceConfig`) — is consolidated in `usecase.go`; the device-level analogue (`NewDeviceFile` wrapping `deviceconfig.Manifest`, `NewDeviceDir`/`ListDeviceIDs` (returns `[]DeviceID`), `DeviceConfig`/`ReadDeviceConfig`/`WriteDeviceConfig`) lives in `device.go`. Pure declarative package — no I/O (the file port's own construction is free; only calling `.Read`/`ports.PatchEncoded` on it touches disk, done from `app/iotedge`). | `models/iotedge/manifesttemplate`, `models/iotedge/deviceconfig` |
| [`models/iotedge/modulesummary/`](models/iotedge/modulesummary) | `Summary`/`NewSummary` (a reduced, read-only module view built from a `manifesttemplate.ModuleConfig`: image, host-mapped `PortBindings`, `Binds`, status, restart policy) plus the declared `ReadReq`/`ReadTool` MCP contract for reading it. Pure declarative package — no I/O. | `models/iotedge/manifesttemplate`, `models/docker` |
| [`models/iotedge/updatemoduleimage/`](models/iotedge/updatemoduleimage) | `Req`/`Tool` — the declared MCP contract for updating one module's container image (input is a plain `ImageURL` string, parsed server-side; output is the module's UPDATED `modulesummary.Summary`). Kept separate from `modulesummary`, mirroring `modulepatch`'s existing separateness from `manifesttemplate`. Pure declarative package — no I/O. | `models/iotedge/manifesttemplate`, `models/iotedge/modulesummary` |
| [`models/iotedge/modulepatch/`](models/iotedge/modulepatch) | `FieldsPatch` — a GENERAL multi-field patch for one module (any subset of `ModuleName`/`Image`/`CreateOptions`/`Env`/`Type`/`Status`/`RestartPolicy`/`Version`) that encodes directly into the manifest's full nested shape (`modulesContent` → `$edgeAgent` → `<key>` → ...), reusing `manifesttemplate`'s own exported field codecs. `FieldsPatchCodec` is a HAND-ROLLED `codex.Codec[T]` (not `codex.Struct`, whose `Encode` always writes every declared field — see its own doc comment) so only the fields actually set are included, leaving everything else — on that module and elsewhere in the manifest — untouched. `EmptyPatchError` (structured, `slog.LogValuer`, deliberately NOT stutter-stripped) signals a caller built a patch with nothing set. `NewUpdateModuleImage(moduleName, image)` is a named smart constructor for the single most common patch operation, validated via `FieldsPatchCodec.New`. Pure declarative package — no I/O. Depends ONLY on `models/iotedge/manifesttemplate` (a sibling subpackage), never on the parent `models/iotedge` directory itself. | `models/iotedge/manifesttemplate` |
...
| [`app/iotedge/`](app/iotedge) | The CONCRETE IMPLEMENTATION built on the `models/iotedge` subpackages' contracts: `ReadUseCase` (reads useCaseName's deployment manifest under basePath via `usecase.NewFile` — named distinctly from `usecase.Read`, which additionally assembles every nested device), `PatchUseCaseModule` (applies a `modulepatch.FieldsPatch` — any subset of one module's fields), `UpdateUseCaseModuleImage` (a thin convenience over `PatchUseCaseModule` for the single most common case, validated via `modulepatch.NewUpdateModuleImage`), `NewReadModuleSummaryToolHandler`/`NewUpdateModuleImageToolHandler` (bind `modulesummary`'s/`updatemoduleimage`'s declared `ReadTool`/`Tool` — sharing an unexported `readModuleSummary` helper for "read manifest, find module, summarize" — returning a `ModuleNotFoundError`, structured `slog.LogValuer`, when the named module is absent; the update handler checks the module exists BEFORE patching, so a nonexistent module fails cleanly instead of merging an incomplete entry). A caller always supplies basePath + useCaseName per call; this package never assumes or hardcodes where a use case's manifest lives. | `models/iotedge/usecase`, `models/iotedge/modulesummary`, `models/iotedge/updatemoduleimage`, `models/iotedge/modulepatch` |
...
files. `models/docker`/`models/iotedge/manifesttemplate` split by DOMAIN
CONCEPT (e.g. `models/docker/image.go`,
`models/iotedge/manifesttemplate/envvars.go`);
...
`registry.NewCredentials`/`NewImageRef`, `manifesttemplate.NewModuleSettings`,
`manifesttemplate.NewEnvVarValueString`/`NewEnvVarValueInt`/`NewEnvVarValueFloat`,
...
constraint (e.g. `docker.CreateOptions`, `manifesttemplate.ModuleConfig`) are
...
```go
manifest, err := format.JSON(manifesttemplate.DeploymentManifestCodec).Unmarshal(manifestJSON)
...
restartPolicy := manifesttemplate.RestartPolicy("on-failure")
