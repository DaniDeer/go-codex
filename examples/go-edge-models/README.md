...
| [`models/iotedge/deviceconfig/`](models/iotedge/deviceconfig) | The PURE WIRE FORMAT of one device's device-specific config file — Go package `deviceconfig`, nested under `models/iotedge`. A real device config file IS a **patch** over its use case's own deployment manifest: `Patch{EdgeAgent map[string]any, EdgeHub map[manifesttemplate.RouteName]manifesttemplate.Route}` — `EdgeAgent` keys are bare dotted paths reaching to ARBITRARY depth inside a module (whole-module, one env var, one settings field, ...); `EdgeHub` keys are whole-route add/override. `PatchCodec` is HAND-ROLLED (dynamic keys); `EmptyPatchError` signals a no-op patch. Deliberately has NO merge logic (see `finaldeviceconfig`). | `models/iotedge/manifesttemplate` (a few field-level codecs only) |
| [`models/iotedge/finaldeviceconfig/`](models/iotedge/finaldeviceconfig) | The DERIVED operation that layers one device's `deviceconfig.Patch` onto its use case's own `manifesttemplate.DeploymentManifest`: `Merge(base, patch) (manifesttemplate.DeploymentManifest, error)` — deep-merges the patch (creating new modules/routes as needed, overwrite/add only, no deletion), then re-validates the result through `manifesttemplate.DeploymentManifestCodec`. Kept as its own package since it depends on BOTH wire formats at once — a dependency shape neither wire package may take on itself. | `models/iotedge/manifesttemplate`, `models/iotedge/deviceconfig` |
| [`models/iotedge/usecase/`](models/iotedge/usecase) | Everything DERIVED/CONSTRUCTED for a "use case" AND the devices nested under it — never wire formats themselves. `Name`/`DeviceID` (named string types, validated via `NameCodec`/`DeviceIDCodec`, constructed via `NewName`/`NewDeviceID`) are this package's typed vocabulary, used throughout instead of bare `string`. The full use-case model/composition — `NewFile(basePath) ports.File[manifesttemplate.DeploymentManifest]` (a declared, TEMPLATED file port over `"{basePath}/usecases/{usecase_name}.json"`), `NewDir`/`ListNames` (returns `[]Name`), and the domain composition `UseCase`/`Read`/`Write` (which also assembles every nested `DeviceConfig`) — is consolidated in `usecase.go`; the device-level analogue (`NewDeviceFile` wrapping `deviceconfig.Patch`, `NewDeviceDir`/`ListDeviceIDs` (returns `[]DeviceID`), `DeviceConfig`/`ReadDeviceConfig`/`WriteDeviceConfig`, `DeviceConfig.Merge(template)` — the "one call" convenience delegating to `finaldeviceconfig.Merge` — and `ReadEffective(basePath, useCaseName, deviceID, opts)`, combining `NewFile.Read`+`ReadDeviceConfig`+`DeviceConfig.Merge` into ONE call, tolerant of a device with no config file yet — that device's effective config is simply the template) lives in `device.go`. Pure declarative package — no I/O (the file port's own construction is free; only calling `.Read`/`ports.PatchEncoded` on it touches disk, done from `app/iotedge`). | `models/iotedge/manifesttemplate`, `models/iotedge/deviceconfig`, `models/iotedge/finaldeviceconfig` |
| [`models/iotedge/modulesummary/`](models/iotedge/modulesummary) | `Summary`/`NewSummary` (a reduced, read-only module view built from a `manifesttemplate.ModuleConfig`: image, host-mapped `PortBindings`, `Binds`, status, restart policy) plus the declared `ReadReq`/`ReadTool` MCP contract for reading it. `ReadReq.DeviceID` is OPTIONAL — set, returns that device's ACTUAL configured summary (template + device config, merged); empty, returns the use case template's own summary. Pure declarative package — no I/O. | `models/iotedge/manifesttemplate`, `models/docker` |
