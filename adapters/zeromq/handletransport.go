...
// NewPublishTransport builds an [events.PublishTransport][T] bound to sock
// — the GENERIC, per-T constructor Decision 7
...
//
//	transport := zeromq.NewPublishTransport[SensorReading](sock, zeromq.PublishOptions[SensorReading]{})
//	err := events.PublishHandle(ctx, ReadingsChannel.WithPublish(events.Publish{}), transport, reading)
func NewPublishTransport[T any](sock FramedSocket, opts PublishOptions[T], formats ...format.Format[T]) events.PublishTransport[T] {
	return &publishTransport[T]{sock: sock, opts: opts, formats: formats}
}
...
func (t *publishTransport[T]) Publish(ctx context.Context, handle *events.ChannelHandle[T], msg T) error {
	return publishHandle(ctx, t.sock, handle, msg, t.opts, t.formats...)
}

// AdapterName implements [events.PublishTransport].
func (t *publishTransport[T]) AdapterName() string { return "zeromq.PublishTransport" }
...
// sock — the GENERIC, per-T constructor Decision 7 uses to satisfy
...
// design rationale.
//
//	transport := zeromq.NewSubscribeTransport[SensorReading](sock, zeromq.SubscribeOptions[SensorReading]{})
...
func NewSubscribeTransport[T any](sock FramedSocket, opts SubscribeOptions[T], formats ...format.Format[T]) events.SubscribeTransport[T] {
	return &subscribeTransport[T]{sock: sock, opts: opts, formats: formats}
}
...
func (t *subscribeTransport[T]) Subscribe(ctx context.Context, handle *events.ChannelHandle[T], fn func(context.Context, T) error) error {
	return subscribeWithHandle(ctx, t.sock, handle, fn, t.opts, t.formats...)
}

// AdapterName implements [events.SubscribeTransport].
func (t *subscribeTransport[T]) AdapterName() string { return "zeromq.SubscribeTransport" }
