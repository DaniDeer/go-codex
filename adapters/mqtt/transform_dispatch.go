...
// pre-handler dispatch point subscribeHandler's own security enforcement
// runs at (D1) — msg is the SAME already-decoded *T the handler will also
...
func dispatchPublishMiddlewareHandlers[T any](ctx context.Context, msg T, handlers []events.ClientMiddlewareHandler) (map[string]string, error) {
	...
	for _, h := range handlers {
		...
		if encErr != nil {
			return nil, middlewareDispatchError{err: encErr, isEncodeErr: true, name: h.Name}
		}
