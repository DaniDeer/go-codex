...
var connectBearerScheme = adaptermqtt.ConnectSecurityScheme{SecurityScheme: route.BasicScheme()}.
	WithCodec(codex.String().Refine(validate.NonEmptyString))

func alwaysFailsScheme() adaptermqtt.ConnectSecurityScheme {
	return adaptermqtt.ConnectSecurityScheme{SecurityScheme: route.BasicScheme()}.
...
func TestNewSecuredClient_ValidCredential_ReturnsWrapper(t *testing.T) {
	client := &mockClient{}
	secured, err := adaptermqtt.NewSecuredClient(client, connectBearerScheme, "svc-account", "s3cr3t")
	if err != nil {
...
func TestNewSecuredClient_MalformedCredential_ReturnsError(t *testing.T) {
	client := &mockClient{}
	secured, err := adaptermqtt.NewSecuredClient(client, alwaysFailsScheme(), "user", "pass")
	if err == nil {
	...
	var credErr adaptermqtt.ConnectSecurityCredentialError
...
func TestNewSecuredClient_NilCodec_NoOp(t *testing.T) {
	client := &mockClient{}
	noCodecScheme := adaptermqtt.ConnectSecurityScheme{SecurityScheme: route.BasicScheme()}
	secured, err := adaptermqtt.NewSecuredClient(client, noCodecScheme, "", "")
	if err != nil {
...
func TestNewSecuredClient_MalformedCredential_ClientNeverUsed(t *testing.T) {
	client := &mockClient{}
	_, err := adaptermqtt.NewSecuredClient(client, alwaysFailsScheme(), "user", "pass")
	if err == nil {
		t.Fatal("want error")
	}
...
func TestSecuredClient_SatisfiesMQTTClient(t *testing.T) {
	var _ pahomqtt.Client = (*adaptermqtt.SecuredClient)(nil)
}
...
func TestSecuredClient_TransparentDelegation(t *testing.T) {
	...
	err = adaptermqtt.Publish(context.Background(), secured, newHandle(), 1, false,
	...
	// client. adaptermqtt.SubscribeHandler builds the pahomqtt.MessageHandler;
	// the caller (here, through the SecuredClient wrapper) registers it
	// with the broker via the client's own Subscribe method.
	handler := adaptermqtt.SubscribeHandler(context.Background(), newHandle(),
...
func TestNewSecuredClient_Observer_RecordsSecurityRejection(t *testing.T) {
	...
	_, err := adaptermqtt.NewSecuredClient(client, alwaysFailsScheme(), "user", "pass", adaptermqtt.WithObserver(obs))
	if err == nil {
		t.Fatal("want error")
	}
...
func TestNewSecuredClient_NilObserver_NoPanic(t *testing.T) {
	client := &mockClient{}
	_, err := adaptermqtt.NewSecuredClient(client, alwaysFailsScheme(), "user", "pass") // no WithObserver
	if err == nil {
		t.Fatal("want error")
	}
}
...
func TestConnectSecurityCredentialError_LogValue(t *testing.T) {
	err := adaptermqtt.ConnectSecurityCredentialError{
...
func TestConnectSecurityCredentialError_ErrorsAs(t *testing.T) {
	...
	wrapped := adaptermqtt.ConnectSecurityCredentialError{Scheme: connectBearerScheme, Err: inner}
	var credErr adaptermqtt.ConnectSecurityCredentialError
