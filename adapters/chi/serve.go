...
// Method+Path — a duplicate chi route registration panic waiting to happen.
...
// [MultiRouteError] — Serve wires NOTHING (zero router registrations) if even
// one fails.
//
...
// registration panic into a returned error.
//
...
func Serve(r gochi.Router, b *rest.Builder) error {
	...
	return wireRoutes(r, toWire)
}

func wireRoutes(r gochi.Router, toWire []wiredRoute) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("chi.Serve: panic wiring routes: %v", r)
		}
	}()
...
// route into it, call Serve, return the resulting router." Reuses Serve's
...
// SAME pipeline `handlerFunc` runs, invoked via reflect.Value.Call since Req/
// Resp are erased at this call site. handle is a *rest.RouteHandle[Req, Resp]
// stored as any.
func buildRouteHandler(handle any) (http.Handler, error) {
	hv := reflect.ValueOf(handle)
	if hv.Kind() != reflect.Pointer || hv.IsNil() {
		return nil, fmt.Errorf("chi.Serve: expected non-nil *rest.RouteHandle[Req, Resp], got %T", handle)
	}
	elem := hv.Elem()
	...
	if handlerFnVal.IsNil() {
		return nil, fmt.Errorf("chi.Serve: route %s %s has no handler (internal error — HasHandler should have skipped it)", descriptor.Method, descriptor.Path)
	}
